...
int fxgmac_phy_force_mode(struct fxgmac_pdata *pdata) {
  ...
  regval = FXGMAC_SET_REG_BITS(regval, PHY_CR_AUTOENG_POS, PHY_CR_AUTOENG_LEN,
                               pdata->phy_autoeng);
  ...
  regval = FXGMAC_SET_REG_BITS(regval, PHY_CR_DUPLEX_POS, PHY_CR_DUPLEX_LEN,
                               pdata->phy_duplex);
 ...
 *      for 1000M capability, bit0=1000M half; bit1=1000M full, refer to mii
 * reg9.[9:8].
 */
...
busy_exit:
  DPRINTK(
      "fxgmac_ephy_autoneg_ability_get exit due to ephy reg access fail.\n");
...
/* this function used to double check the speed. for fiber, to correct there is
 * no 10M */
 ...
 *      media: only valid when ret_link=1, (YT8614_SMI_SEL_SDS_SGMII + 1) for
 * fiber; (YT8614_SMI_SEL_PHY + 1) for utp. -1 for link down.
 */
...
static void fxgmac_phy_update_link(struct net_device *netdev) {
  ...
  cur_link = FXGMAC_GET_REG_BITS(regval, MGMT_EPHY_CTRL_STA_EPHY_LINKUP_POS,
...
#endif
{
...
#endif
  {
    ...
    DPRINTK("fxgmac_phy_timer polling, powerstate changed, %ld, netdev=%lx, "
            "tm=%lx\n",
            pdata->expansion.powerstate, (unsigned long)(pdata->netdev),
...
#else
  init_timer_key(&pdata->expansion.phy_poll_tm, 0, "fuxi_phy_link_update_timer",
                 NULL);
#endif
