...
static void fxgmac_unmap_desc_data(struct fxgmac_pdata *pdata,
  ...
  if (desc_data->skb_dma) {
    if (desc_data->mapped_as_page) {
      dma_unmap_page(pdata->dev, desc_data->skb_dma, desc_data->skb_dma_len,
                     DMA_TO_DEVICE);
    } else {
      dma_unmap_single(pdata->dev, desc_data->skb_dma, desc_data->skb_dma_len,
                       DMA_TO_DEVICE);
    }
    desc_data->skb_dma = 0;
  ...
  if (desc_data->rx.hdr.pa_unmap.pages) {
    dma_unmap_page(pdata->dev, desc_data->rx.hdr.pa_unmap.pages_dma,
                   desc_data->rx.hdr.pa_unmap.pages_len, DMA_FROM_DEVICE);
  ...
  if (desc_data->rx.buf.pa_unmap.pages) {
    dma_unmap_page(pdata->dev, desc_data->rx.buf.pa_unmap.pages_dma,
                   desc_data->rx.buf.pa_unmap.pages_len, DMA_FROM_DEVICE);
...
static int fxgmac_init_ring(struct fxgmac_pdata *pdata,
  ...
  ring->desc_data_head =
      kcalloc(dma_desc_count, sizeof(struct fxgmac_desc_data), GFP_KERNEL);
  if (!ring->desc_data_head)
    return -ENOMEM;
...
static int fxgmac_alloc_rings(struct fxgmac_pdata *pdata) {
  ...
  for (i = 0; i < pdata->channel_count; i++, channel++) {
    ...
    if (i < pdata->tx_ring_count) {
      ret = fxgmac_init_ring(pdata, channel->tx_ring, pdata->tx_desc_count);
    ...
    ret = fxgmac_init_ring(pdata, channel->rx_ring, pdata->rx_desc_count);
    if (ret) {
      netdev_alert(pdata->netdev, "error initializing Rx ring\n");
      goto err_init_ring;
    }
...
static void fxgmac_free_channels(struct fxgmac_pdata *pdata) {
  if (!pdata->channel_head)
    return;
  if (netif_msg_drv(pdata)) {
    DPRINTK("free_channels,tx_ring=%p", pdata->channel_head->tx_ring);
  }
  ...
  // comment out below line for that, channel_count is initialized only once in
  // hw_init() pdata->channel_count = 0;
}
#else
static void fxgmac_free_channels(struct fxgmac_pdata *pdata) {
  if (!pdata->channel_head)
...
static int fxgmac_alloc_channels(struct fxgmac_pdata *pdata) {
  ...
  Status = adpt->Io_Function->AllocateBuffer(
      adpt->Io_Function, AllocateAnyPages, EfiRuntimeServicesData,
      UNDI_MEM_PAGES(CHANNEL_MEMORY_NEEDED), (VOID **)&(adpt->MemoryChannelPtr),
      0);
  ...
  DEBUGPRINT(INIT, "[%a,%a,%d]:status=%x,MemoryChannelPtr=%llx\n", __FILE__,
             __func__, __LINE__, Status, adpt->MemoryChannelPtr);
  ...
  DEBUGPRINT(INIT, "[%a,%a,%d]:Channel_head=%llx,vir_addr=%llx\n", __FILE__,
             __func__, __LINE__, channel_head, vir_addr);
  netif_dbg(pdata, drv, pdata->netdev, "channel_head=%p\n", channel_head);

  vir_addr = (vir_addr + 4 * sizeof(struct fxgmac_channel) + cache_sz) &
             (~(cache_sz - 1));
#ifdef UEFI_64
  ...
  vir_addr = (vir_addr + 4 * sizeof(struct fxgmac_ring) + cache_sz) &
             (~(cache_sz - 1));
#ifdef UEFI_64
  ...
  DEBUGPRINT(INIT,
             "[%a,%a,%d]:Channel_head=%llx,*channel_head=%llx,vir_addr=%llx,tx_"
             "ring=%llx,rx_ring=%llx,channelcount=%llx,pdata=%llx\n",
             __FILE__, __func__, __LINE__, channel_head, *channel_head,
             vir_addr, tx_ring, rx_ring, pdata->channel_count,
             &channel_head->pdata);
  for (i = 0, channel = channel_head; i < pdata->channel_count;
       i++, channel++) {
    // snprintf(channel->name, sizeof(channel->name), "channel-%u", i);
    // RtlStringCchPrintfA(channel->name, sizeof(channel->name), "channel-%u",
    // i); netif_dbg(pdata, drv, pdata->netdev,"channel-%u\n", i);

    DEBUGPRINT(INIT, "[%a,%a,%d]:channel=%llx,&channel->pdata=%llx\n", __FILE__,
               __func__, __LINE__, channel, &channel->pdata);
    channel->pdata = pdata;
    DEBUGPRINT(INIT, "[%a,%d]:channel->pdata:%llx,pdata:%llx\n", __func__,
               __LINE__, channel->pdata, pdata);
    ...
    DEBUGPRINT(INIT,
               "[%a,%d]:Channel_dma_regs=%llx,channel txring=%llx,channel "
               "rx_ring=%llx,i=%llx,channel_count=%llx\n",
               __func__, __LINE__, channel->dma_regs, channel->tx_ring,
               channel->rx_ring, i, pdata->channel_count);
  }
...
static int fxgmac_alloc_channels(struct fxgmac_pdata *pdata) {
  ...
  channel_head =
      kcalloc(pdata->channel_count, sizeof(struct fxgmac_channel), GFP_KERNEL);
  if (netif_msg_drv(pdata)) {
    DPRINTK("alloc_channels,channel_head=%p,size=%d*%d\n", channel_head,
            pdata->channel_count, (u32)sizeof(struct fxgmac_channel));
  }
  ...
  tx_ring =
      kcalloc(pdata->tx_ring_count, sizeof(struct fxgmac_ring), GFP_KERNEL);
  if (!tx_ring)
    goto err_tx_ring;
  ...
  rx_ring =
      kcalloc(pdata->rx_ring_count, sizeof(struct fxgmac_ring), GFP_KERNEL);
  if (!rx_ring)
    goto err_rx_ring;
  ...
  // DPRINTK("fxgmac_alloc_channels
  // ch_num=%d,rxring=%d,txring=%d\n",pdata->channel_count,
  // pdata->rx_ring_count,pdata->tx_ring_count);
    ...
    if (pdata->per_channel_irq) {
      ...
      if (msix) {
        ...
        if (FXGMAC_IS_CHANNEL_WITH_TX_IRQ(i)) {
          pdata->channel_irq[FXGMAC_MAX_DMA_CHANNELS] =
              pdata->expansion.msix_entries[FXGMAC_MAX_DMA_CHANNELS].vector;

          if (pdata->channel_irq[FXGMAC_MAX_DMA_CHANNELS] < 0) {
            netdev_err(pdata->netdev, "get_irq %u for tx failed\n", i + 1);
            goto err_irq;
          }
...
static int fxgmac_alloc_channels(struct fxgmac_pdata *pdata) {
  ...
  for (i = 0, channel = channel_head; i < pdata->channel_count;
    ...
    netif_dbg(pdata, drv, pdata->netdev,
              ...
              channel->dma_regs, channel->tx_ring, channel->rx_ring);
  }
...
static int fxgmac_alloc_pages(struct fxgmac_pdata *pdata,
  ...
  /* Map the pages */
  pages_dma =
      dma_map_page(pdata->dev, pages, 0, PAGE_SIZE << order, DMA_FROM_DEVICE);
  if (dma_mapping_error(pdata->dev, pages_dma)) {
    put_page(pages);
    return -ENOMEM;
  }
...
static int fxgmac_map_rx_buffer(struct fxgmac_pdata *pdata,
  ...
  if (!ring->rx_hdr_pa.pages) {
    ret = fxgmac_alloc_pages(pdata, &ring->rx_hdr_pa, GFP_ATOMIC, 0);
    if (ret)
      return ret;
  }
...
static void fxgmac_tx_desc_init(struct fxgmac_pdata *pdata) {
  ...
  writereg(pdata->pAdapter, NIC_DEF_TBDS - 1,
  ...
  writereg(pdata->pAdapter,
           upper_32_bits(cpu_to_le64(bus_to_phys(
               pdata->pdev,
               (pci_addr_t)(unsigned long)
                   pdata->expansion.tx_desc_list))), // adpt->TbdPhyAddr
           FXGMAC_DMA_REG(channel, DMA_CH_TDLR_HI));
  writereg(pdata->pAdapter,
           lower_32_bits(cpu_to_le64(bus_to_phys(
               pdata->pdev,
               (pci_addr_t)(unsigned long)
                   pdata->expansion.tx_desc_list))), // adpt->TbdPhyAddr
           FXGMAC_DMA_REG(channel, DMA_CH_TDLR_LO));
...
static void fxgmac_rx_desc_init(struct fxgmac_pdata *pdata) {
  ...
  for (i = 0; i < NIC_DEF_RECV_BUFFERS; i++) {
    ...
    fxgmac_desc_reset(desc_data);
    desc_data->desc0 = lower_32_bits(bus_to_phys(
        pdata->pdev, (pci_addr_t)(unsigned long)(pdata->expansion.rx_buffer)));
    desc_data->desc1 = upper_32_bits(bus_to_phys(
        pdata->pdev, (pci_addr_t)(unsigned long)(pdata->expansion.rx_buffer)));
    desc_data->desc3 =
        FXGMAC_SET_REG_BITS_LE(desc_data->desc3, RX_NORMAL_DESC3_BUF2V_POS,
                               RX_NORMAL_DESC3_BUF2V_LEN, 1);
    desc_data->desc3 =
        FXGMAC_SET_REG_BITS_LE(desc_data->desc3, RX_NORMAL_DESC3_BUF1V_POS,
                               RX_NORMAL_DESC3_BUF1V_LEN, 1);
    desc_data->desc3 = FXGMAC_SET_REG_BITS_LE(
        desc_data->desc3, RX_NORMAL_DESC3_OWN_POS, RX_NORMAL_DESC3_OWN_LEN, 1);
  }

  /* Update the total number of Rx descriptors */
  writereg(pdata->pAdapter, NIC_DEF_RECV_BUFFERS - 1,
  ...
  HwRbdPa = (uint64_t)pdata->expansion.rx_desc_list +
            (NIC_DEF_RECV_BUFFERS) * sizeof(struct fxgmac_dma_desc);
  /* Update the Rx Descriptor Tail Pointer */
  writereg(pdata->pAdapter, lower_32_bits((unsigned long)HwRbdPa),
...
static void fxgmac_tx_desc_init_channel(struct fxgmac_channel *channel) {
...
#elif defined(UEFI)
  writereg(
      channel->pdata->pAdapter,
      GetPhyAddrHigh(
          ((PADAPTER)channel->pdata->pAdapter)->TbdPhyAddr), // adpt->TbdPhyAddr
      FXGMAC_DMA_REG(channel, DMA_CH_TDLR_HI));
  writereg(channel->pdata->pAdapter,
           GetPhyAddrLow(((PADAPTER)channel->pdata->pAdapter)->TbdPhyAddr),
           FXGMAC_DMA_REG(channel, DMA_CH_TDLR_LO));
#elif defined(PXE)
  ...
  writereg(
      channel->pdata->pAdapter,
      upper_32_bits(((PMOTORCOMM_ADAPTER)channel->pdata->pAdapter)->TbdPhyAddr),
      FXGMAC_DMA_REG(channel, DMA_CH_TDLR_HI));
  writereg(
      channel->pdata->pAdapter,
      lower_32_bits(((PMOTORCOMM_ADAPTER)channel->pdata->pAdapter)->TbdPhyAddr),
      FXGMAC_DMA_REG(channel, DMA_CH_TDLR_LO));
#else // netadaptercx & ndis
  writereg(channel->pdata->pAdapter,
           NdisGetPhysicalAddressHigh(
               ((PMP_ADAPTER)channel->pdata->pAdapter)->HwTbdBasePa),
           FXGMAC_DMA_REG(channel, DMA_CH_TDLR_HI));
  writereg(channel->pdata->pAdapter,
...
static void fxgmac_rx_desc_reset(struct fxgmac_pdata *pdata,
  ...
  dma_desc->desc3 = FXGMAC_SET_REG_BITS_LE(
      dma_desc->desc3, RX_NORMAL_DESC3_INTE_POS, RX_NORMAL_DESC3_INTE_LEN, 1);
  dma_desc->desc3 = FXGMAC_SET_REG_BITS_LE(
      dma_desc->desc3, RX_NORMAL_DESC3_BUF2V_POS, RX_NORMAL_DESC3_BUF2V_LEN, 0);
  dma_desc->desc3 = FXGMAC_SET_REG_BITS_LE(
      dma_desc->desc3, RX_NORMAL_DESC3_BUF1V_POS, RX_NORMAL_DESC3_BUF1V_LEN, 1);
  ...
  dma_desc->desc3 = FXGMAC_SET_REG_BITS_LE(
      dma_desc->desc3, RX_NORMAL_DESC3_OWN_POS, RX_NORMAL_DESC3_OWN_LEN, 1);
...
static void fxgmac_rx_desc_init_channel(struct fxgmac_channel *channel) {
  ...
  /* Update the Rx Descriptor Tail Pointer */
  desc_data =
      FXGMAC_GET_DESC_DATA(ring, start_index + ring->dma_desc_count - 1);
  writereg(pdata->pAdapter, lower_32_bits(desc_data->dma_desc_addr),
           FXGMAC_DMA_REG(channel, DMA_CH_RDTR_LO));
#elif defined(UEFI)
  ...
  /* Update the starting address of descriptor ring */
  writereg(pdata->pAdapter,
           GetPhyAddrHigh(((PADAPTER)channel->pdata->pAdapter)->RbdPhyAddr),
           FXGMAC_DMA_REG(channel, DMA_CH_RDLR_HI));
  writereg(pdata->pAdapter,
           GetPhyAddrLow(((PADAPTER)channel->pdata->pAdapter)->RbdPhyAddr),
           FXGMAC_DMA_REG(channel, DMA_CH_RDLR_LO));

  HwRbdPa = ((PADAPTER)channel->pdata->pAdapter)->RbdPhyAddr +
            (pdata->rx_desc_count - 1) * sizeof(struct fxgmac_dma_desc);

  /* Update the Rx Descriptor Tail Pointer */
  writereg(pdata->pAdapter, GetPhyAddrLow(HwRbdPa),
           FXGMAC_DMA_REG(channel, DMA_CH_RDTR_LO));
#elif defined(PXE)
#elif defined(UBOOT)
#elif defined(DPDK)
  ...
  writereg(
      channel->pdata->pAdapter,
      upper_32_bits(((PMOTORCOMM_ADAPTER)channel->pdata->pAdapter)->RdbPhyAddr),
      FXGMAC_DMA_REG(channel, DMA_CH_RDLR_HI));
  writereg(
      channel->pdata->pAdapter,
      lower_32_bits(((PMOTORCOMM_ADAPTER)channel->pdata->pAdapter)->RdbPhyAddr),
      FXGMAC_DMA_REG(channel, DMA_CH_RDLR_LO));

  HwRbdPa.QuadPart =
      ((PMOTORCOMM_ADAPTER)channel->pdata->pAdapter)->RdbPhyAddr.QuadPart +
      (channel->pdata->rx_desc_count - 1) * sizeof(HW_RBD);

  writereg(channel->pdata->pAdapter, HwRbdPa.LowPart,
           FXGMAC_DMA_REG(channel, DMA_CH_RDTR_LO));
#else // netadaptercx & ndis
  ...
  writereg(
      channel->pdata->pAdapter,
      ((PMP_ADAPTER)channel->pdata->pAdapter)->RxQueue[Qid].NumHwRecvBuffers -
          1,
      FXGMAC_DMA_REG(channel, DMA_CH_RDRLR));
  ...
  writereg(
      channel->pdata->pAdapter,
      NdisGetPhysicalAddressHigh(
          ((PMP_ADAPTER)channel->pdata->pAdapter)->RxQueue[Qid].HwRbdBasePa),
      FXGMAC_DMA_REG(channel, DMA_CH_RDLR_HI));
  writereg(
      channel->pdata->pAdapter,
      NdisGetPhysicalAddressLow(
...
#else
  HwRbdPa.QuadPart =
      ((PMP_ADAPTER)channel->pdata->pAdapter)
          ->RxQueue[Qid]
          .HwRbdBasePa.QuadPart +
      (((PMP_ADAPTER)channel->pdata->pAdapter)->RxQueue[Qid].NumHwRecvBuffers -
       1) *
          sizeof(HW_RBD);

  /* Update the Rx Descriptor Tail Pointer */
  writereg(channel->pdata->pAdapter, NdisGetPhysicalAddressLow(HwRbdPa),
           FXGMAC_DMA_REG(channel, DMA_CH_RDTR_LO));
#endif
#endif
}
...
static int fxgmac_map_tx_skb(struct fxgmac_channel *channel,
  ...
  if (tso) {
    /* Map the TSO header */
    skb_dma = dma_map_single(pdata->dev, skb->data, pkt_info->header_len,
                             DMA_TO_DEVICE);
  ...
  for (datalen = skb_headlen(skb) - offset; datalen;) {
    ...
    skb_dma =
        dma_map_single(pdata->dev, skb->data + offset, len, DMA_TO_DEVICE);
    if (dma_mapping_error(pdata->dev, skb_dma)) {
      netdev_alert(pdata->netdev, "dma_map_single failed\n");
      goto err_out;
    }
...
#if 1
      skb_dma = skb_frag_dma_map(pdata->dev, frag, offset, len, DMA_TO_DEVICE);
#else
      skb_dma = dma_map_single(pdata->dev, addr + offset, len, DMA_TO_DEVICE);
#endif
      if (dma_mapping_error(pdata->dev, skb_dma)) {
        netdev_alert(pdata->netdev, "skb_frag_dma_map failed\n");
        goto err_out;
      }
      desc_data->skb_dma = skb_dma;
      desc_data->skb_dma_len = len;
   ...
   */
  desc_data =
      FXGMAC_GET_DESC_DATA(ring, (cur_index - 1) & (ring->dma_desc_count - 1));
  desc_data->skb = skb;
