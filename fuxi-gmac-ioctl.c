...
static void fxgmac_dbg_tx_pkt(struct fxgmac_pdata *pdata, u8 *pcmd_data) {
  ...
  if (skb->len > 1514) {
    ...
    tcpHeadLen =
        (pSkb_data[TEST_MAC_HEAD + ipHeadLen + TEST_TCP_HEAD_LEN_OFFSET] >> 4 &
         0xF) *
        4;
    ...
    // DPRINTK("offload_len is %d, skb_shinfo(skb)->gso_size is %d",
    ...
    if (offload_len % skb_shinfo(skb)->gso_size == 0) {
      ...
      pdata->expansion.fxgmac_test_last_tso_len =
          offload_len % skb_shinfo(skb)->gso_size + headTotalLen;
    }
...
static void fxgmac_dbg_rx_pkt(struct fxgmac_pdata *pdata, u8 *pcmd_data) {
  ...
  while (pdata->expansion.fxgmac_test_skb_arr_in_index !=
    ...
    rx_skb = pdata->expansion.fxgmac_test_skb_array
                 [pdata->expansion.fxgmac_test_skb_arr_out_index];
    ...
    if (rx_skb->len + sizeof(fxgmac_test_packet) + totalLen < 64000) {
      ...
      /* update next pointer */
      if ((pdata->expansion.fxgmac_test_skb_arr_out_index + 1) %
              FXGMAC_MAX_DBG_TEST_PKT ==
          pdata->expansion.fxgmac_test_skb_arr_in_index) {
        pkt.next = NULL;
      } else {
        pkt.next =
            (pfxgmac_test_packet)(addr + totalLen + sizeof(fxgmac_test_packet) +
                                  pkt.length);
      }

      /* copy data to user space */
      if (copy_to_user((void *)(addr + totalLen), (void *)(&pkt),
                       sizeof(fxgmac_test_packet))) {
        DPRINTK("cppy pkt data to user fail...");
      }
      // FXGMAC_PR("FXG:rx_skb->len=%d", rx_skb->len);
      if (copy_to_user((void *)(addr + totalLen + sizeof(fxgmac_test_packet)),
      ...
      pdata->expansion.fxgmac_test_skb_array
          [pdata->expansion.fxgmac_test_skb_arr_out_index] = NULL;

      /* update gCurSkbOutIndex */
      pdata->expansion.fxgmac_test_skb_arr_out_index =
          (pdata->expansion.fxgmac_test_skb_arr_out_index + 1) %
          FXGMAC_MAX_DBG_TEST_PKT;
    } else {
  ...
  if (arg != 0) {
    ...
    case FXGMAC_DFS_IOCTL_DIAG_BEGIN:
      ...
      while (ex->fxgmac_test_skb_arr_in_index !=
        ...
        ex->fxgmac_test_skb_arr_out_index =
            (ex->fxgmac_test_skb_arr_out_index + 1) % FXGMAC_MAX_DBG_TEST_PKT;
      }
    ...
    case FXGMAC_EFUSE_WRITE_PATCH_PER_INDEX:
      ...
      /*
       * DPRINTK("FXGMAC_EFUSE_WRITE_PATCH_PER_INDEX, index = %d, address =
    ...
    case FXGMAC_EFUSE_READ_PATCH_PER_INDEX:
      ...
      ret = hw_ops->read_patch_from_efuse_per_index(
          pdata, ex_data.val0, &ex_data.val1, &ex_data.val2);
      /*
       * DPRINTK("FXGMAC_EFUSE_READ_PATCH_PER_INDEX, address = 0x%x, val =
       * 0x%x\n", ex_data.val1, ex_data.val2);
       */
      if (ret) {
        memcpy(data, &ex_data, sizeof(CMD_DATA));
    ...
    case FXGMAC_GET_SUBSYS_ID:
      memcpy(&ex_data, data, sizeof(CMD_DATA));
      ret =
          hw_ops->read_mac_subsys_from_efuse(pdata, NULL, &ex_data.val0, NULL);
      if (ret) {
    ...
    case FXGMAC_SET_SUBSYS_ID:
      memcpy(&ex_data, data, sizeof(CMD_DATA));
      ret = hw_ops->write_mac_subsys_to_efuse(pdata, NULL, &ex_data.val0, NULL);
      break;
    ...
    case FXGMAC_SET_REG:
      ...
      regval = hw_ops->set_gmac_register(
          pdata, (u8 *)(pdata->base_mem + ex_data.val0), ex_data.val1);
    ...
    case FXGMAC_SET_RX_MODERATION:
      ...
      regval = FXGMAC_SET_REG_BITS(regval, INT_MOD_RX_POS, INT_MOD_RX_LEN,
                                   ex_data.val0);
    ...
    case FXGMAC_SET_TX_MODERATION:
      memcpy(&ex_data, data, sizeof(CMD_DATA));
      regval = readreg(pdata->pAdapter, pdata->base_mem + INT_MOD);
      regval = FXGMAC_SET_REG_BITS(regval, INT_MOD_TX_POS, INT_MOD_TX_LEN,
                                   ex_data.val0);
      writereg(pdata->pAdapter, regval, pdata->base_mem + INT_MOD);
      break;
    ...
    case FXGMAC_GET_TXRX_MODERATION:
      regval = readreg(pdata->pAdapter, pdata->base_mem + INT_MOD);
      ex_data.val0 =
          FXGMAC_GET_REG_BITS(regval, INT_MOD_RX_POS, INT_MOD_RX_LEN);
      ex_data.val1 =
          FXGMAC_GET_REG_BITS(regval, INT_MOD_TX_POS, INT_MOD_TX_LEN);
      memcpy(data, &ex_data, sizeof(CMD_DATA));
      out_total_size = ioctl_cmd_size + sizeof(CMD_DATA);
      if (copy_to_user((void *)arg, (void *)buf, out_total_size))
        goto err;
