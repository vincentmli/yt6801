...
int fxgmac_init(struct fxgmac_pdata *pdata, bool save_private_reg) {
  ...
  pdata->tx_ring_count =
      min_t(unsigned int, num_online_cpus(), pdata->hw_feat.tx_ch_cnt);
  pdata->tx_ring_count =
      min_t(unsigned int, pdata->tx_ring_count, pdata->hw_feat.tx_q_cnt);
  ...
  pdata->rx_ring_count = min_t(unsigned int, netif_get_num_default_rss_queues(),
...
#else
  pdata->rx_ring_count =
      min_t(unsigned int, pdata->rx_ring_count, pdata->hw_feat.rx_q_cnt);
#endif
  ...
  DPRINTK(
      "default rss queues:%u, rx_ch_cnt:%u, rx_q_cnt:%u, rx_ring_count:%u\n",
  ...
  for (i = 0; i < FXGMAC_RSS_MAX_TABLE_SIZE; i++) {
    pdata->rss_table[i] = FXGMAC_SET_REG_BITS(
        pdata->rss_table[i], MAC_RSSDR_DMCH_POS, MAC_RSSDR_DMCH_LEN,
        i % pdata->rx_ring_count); // note, rx_ring_count should be equal to IRQ
                                   // requsted for MSIx, 4
  }
#endif

  pdata->rss_options = FXGMAC_SET_REG_BITS(
      pdata->rss_options, MAC_RSSCR_IP4TE_POS, MAC_RSSCR_IP4TE_LEN, 1);
  pdata->rss_options = FXGMAC_SET_REG_BITS(
      pdata->rss_options, MAC_RSSCR_TCP4TE_POS, MAC_RSSCR_TCP4TE_LEN, 1);
  pdata->rss_options = FXGMAC_SET_REG_BITS(
      pdata->rss_options, MAC_RSSCR_UDP4TE_POS, MAC_RSSCR_UDP4TE_LEN, 1);
...
#ifndef RHEL_MAJOR
      rc = pci_enable_msix_range(pdata->pdev, pdata->expansion.msix_entries,
                                 req_vectors, req_vectors);
#else
#if (RHEL_RELEASE_CODE >= RHEL_RELEASE_VERSION(7, 0)) &&                       \
    (RHEL_RELEASE_CODE < RHEL_RELEASE_VERSION(7, 8))
      rc = pci_enable_msix(pdata->pdev, pdata->expansion.msix_entries,
                           req_vectors);
#else
      rc = pci_enable_msix_range(pdata->pdev, pdata->expansion.msix_entries,
                                 req_vectors, req_vectors);
      ...
      if (req_vectors >= FXGMAC_MAX_DMA_CHANNELS_PLUS_1TX) {
        ...
        pdata->expansion.int_flags = FXGMAC_SET_REG_BITS(
            ...
            FXGMAC_FLAG_INTERRUPT_LEN, FXGMAC_FLAG_MSIX_ENABLED);
...
int fxgmac_drv_probe(struct device *dev, struct fxgmac_resources *res) {
  ...
  netdev =
      alloc_etherdev_mq(sizeof(struct fxgmac_pdata), FXGMAC_MAX_DMA_CHANNELS);
...
void fxgmac_dump_tx_desc(struct fxgmac_pdata *pdata, struct fxgmac_ring *ring,
  ...
  while (count--) {
    ...
    netdev_dbg(pdata->netdev, "TX_NORMAL_DESC[%d %s] = %08x:%08x:%08x:%08x\n",
               idx, (flag == 1) ? "QUEUED FOR TX" : "TX BY DEVICE",
...
void fxgmac_dbg_pkt(struct net_device *netdev, struct sk_buff *skb,
                    bool tx_rx) {
  ...
  for (i = 0; i < skb->len; i += 32) {
    ...
    hex_dump_to_buffer(&skb->data[i], len, 32, 1, buffer, sizeof(buffer),
                       false);
...
void fxgmac_get_all_hw_features(struct fxgmac_pdata *pdata) {
  ...
  hw_feat->phyifsel = FXGMAC_GET_REG_BITS(mac_hfr0, MAC_HWF0R_ACTPHYIFSEL_POS,
                                          MAC_HWF0R_ACTPHYIFSEL_LEN);
  hw_feat->vlhash =
      FXGMAC_GET_REG_BITS(mac_hfr0, MAC_HWF0R_VLHASH_POS, MAC_HWF0R_VLHASH_LEN);
  hw_feat->sma =
      FXGMAC_GET_REG_BITS(mac_hfr0, MAC_HWF0R_SMASEL_POS, MAC_HWF0R_SMASEL_LEN);
  hw_feat->rwk =
      FXGMAC_GET_REG_BITS(mac_hfr0, MAC_HWF0R_RWKSEL_POS, MAC_HWF0R_RWKSEL_LEN);
  hw_feat->mgk =
      FXGMAC_GET_REG_BITS(mac_hfr0, MAC_HWF0R_MGKSEL_POS, MAC_HWF0R_MGKSEL_LEN);
  hw_feat->mmc =
      FXGMAC_GET_REG_BITS(mac_hfr0, MAC_HWF0R_MMCSEL_POS, MAC_HWF0R_MMCSEL_LEN);
  hw_feat->aoe = FXGMAC_GET_REG_BITS(mac_hfr0, MAC_HWF0R_ARPOFFSEL_POS,
                                     MAC_HWF0R_ARPOFFSEL_LEN);
  hw_feat->ts =
      FXGMAC_GET_REG_BITS(mac_hfr0, MAC_HWF0R_TSSEL_POS, MAC_HWF0R_TSSEL_LEN);
  hw_feat->eee =
      FXGMAC_GET_REG_BITS(mac_hfr0, MAC_HWF0R_EEESEL_POS, MAC_HWF0R_EEESEL_LEN);
  ...
  hw_feat->sa_vlan_ins = FXGMAC_GET_REG_BITS(mac_hfr0, MAC_HWF0R_SAVLANINS_POS,
  ...
  hw_feat->adv_ts_hi = FXGMAC_GET_REG_BITS(mac_hfr1, MAC_HWF1R_ADVTHWORD_POS,
                                           MAC_HWF1R_ADVTHWORD_LEN);
  hw_feat->dma_width =
      FXGMAC_GET_REG_BITS(mac_hfr1, MAC_HWF1R_ADDR64_POS, MAC_HWF1R_ADDR64_LEN);
  hw_feat->dcb =
      FXGMAC_GET_REG_BITS(mac_hfr1, MAC_HWF1R_DCBEN_POS, MAC_HWF1R_DCBEN_LEN);
  hw_feat->sph =
      FXGMAC_GET_REG_BITS(mac_hfr1, MAC_HWF1R_SPHEN_POS, MAC_HWF1R_SPHEN_LEN);
#if (0 /*LINUX_VERSION_CODE >= KERNEL_VERSION(5,0,0)*/)
  hw_feat->tso = 0; // bypass tso
#else
  hw_feat->tso =
      FXGMAC_GET_REG_BITS(mac_hfr1, MAC_HWF1R_TSOEN_POS, MAC_HWF1R_TSOEN_LEN);
#endif
  hw_feat->dma_debug = FXGMAC_GET_REG_BITS(mac_hfr1, MAC_HWF1R_DBGMEMA_POS,
  ...
  hw_feat->tc_cnt = 3 /*1, yzhang try,0412*/; /* FXGMAC_GET_REG_BITS(mac_hfr1,
                ...
                MAC_HWF1R_NUMTC_LEN); */
  hw_feat->avsel =
      FXGMAC_GET_REG_BITS(mac_hfr1, MAC_HWF1R_AVSEL_POS, MAC_HWF1R_AVSEL_LEN);
  hw_feat->ravsel =
      FXGMAC_GET_REG_BITS(mac_hfr1, MAC_HWF1R_RAVSEL_POS, MAC_HWF1R_RAVSEL_LEN);
...
#if 1 /* hw implement only 1 Q, but from software we see 4 logical Qs.         \
         hardcode to 4 Qs. */
...
#else
  hw_feat->rx_q_cnt =
      FXGMAC_GET_REG_BITS(mac_hfr2, MAC_HWF2R_RXQCNT_POS, MAC_HWF2R_RXQCNT_LEN);
#endif
  hw_feat->tx_q_cnt =
      FXGMAC_GET_REG_BITS(mac_hfr2, MAC_HWF2R_TXQCNT_POS, MAC_HWF2R_TXQCNT_LEN);
...
#else
  hw_feat->rx_ch_cnt = FXGMAC_GET_REG_BITS(mac_hfr2, MAC_HWF2R_RXCHCNT_POS,
                                           MAC_HWF2R_RXCHCNT_LEN);
#endif
  hw_feat->tx_ch_cnt = FXGMAC_GET_REG_BITS(mac_hfr2, MAC_HWF2R_TXCHCNT_POS,
                                           MAC_HWF2R_TXCHCNT_LEN);
  hw_feat->pps_out_num = FXGMAC_GET_REG_BITS(mac_hfr2, MAC_HWF2R_PPSOUTNUM_POS,
