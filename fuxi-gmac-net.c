...
static void fxgmac_update_esd_stats(struct fxgmac_pdata *pdata) {
  ...
  value =
      readreg(pdata->pAdapter, pdata->mac_regs + MMC_TXEXCESSIVECOLLSIONFRAMES);
...
static void fxgmac_esd_work(struct work_struct *work) {
  struct fxgmac_pdata *pdata =
      container_of(work, struct fxgmac_pdata, expansion.esd_work.work);
...
unsigned int fxgmac_get_netdev_ip4addr(struct fxgmac_pdata *pdata) {
  ...
  if (ifa) {
    ...
    DPRINTK("%s, netdev %s IPv4 address %pI4, mask: %pI4\n", __FUNCTION__,
            ifa->ifa_label, &ifa->ifa_address, &ifa->ifa_mask);
  }
...
unsigned char *fxgmac_get_netdev_ip6addr(struct fxgmac_pdata *pdata,
  ...
  // in6_pton("fe80::35c6:dd1b:9745:fc9b", -1, (u8*)ipval, -1, NULL); //here
  // just hard code for default
  if (ipval) {
  ...
  if (i6dev != NULL) {
    ...
    list_for_each_entry(ifp, &i6dev->addr_list, if_list) {
      ...
      if (((ifa_flag & FXGMAC_NS_IFA_GLOBAL_UNICAST) && (ifp->scope != IFA_LINK)) || ((ifa_flag &
...
!(ifp->flags & IFA_F_TENTATIVE)*/)) {
        ...
        // netdev->name, addr_ip6); DPRINTK("%s, netdev %s IPv6 solicited-node
  ...
  // DPRINTK("%s, netdev %s IPv6 local-link address %pI6\n",__FUNCTION__,
  // netdev->name, addr_ip6); DPRINTK("%s, netdev %s IPv6 solicited-node adr
  // %pI6\n",__FUNCTION__, netdev->name, addr_ip6_solicited);
...
static void fxgmac_prep_tx_pkt(struct fxgmac_pdata *pdata,
  ...
  if (netif_msg_tx_done(pdata)) {
    DPRINTK(
        "fxgmac_prep_tx_pkt callin,pkt desc cnt=%d,skb len=%d, skbheadlen=%d\n",
  ...
  if (fxgmac_is_tso(skb)) {
    ...
    if (netif_msg_tx_done(pdata)) {
      DPRINTK("fxgmac_is_tso=%d, ip_summed=%d,skb gso=%d\n",
              ((skb->ip_summed == CHECKSUM_PARTIAL) && (skb_is_gso(skb))) ? 1
                                                                          : 0,
    ...
    pkt_info->attributes = FXGMAC_SET_REG_BITS(
        pkt_info->attributes, TX_PACKET_ATTRIBUTES_CSUM_ENABLE_POS,
  ...
  if (skb_vlan_tag_present(skb)) {
    ...
    pkt_info->attributes = FXGMAC_SET_REG_BITS(
        pkt_info->attributes, TX_PACKET_ATTRIBUTES_VLAN_CTAG_POS,
  ...
  if (netif_msg_tx_done(pdata)) {
    DPRINTK("fxgmac_prep_tx_pkt callout,pkt desc cnt=%d,skb len=%d, "
            "skbheadlen=%d,frags=%d\n",
            pkt_info->desc_count, skb->len, skb_headlen(skb),
            skb_shinfo(skb)->nr_frags);
  }
}

static int fxgmac_calc_rx_buf_size(struct net_device *netdev,
                                   unsigned int mtu) {
  unsigned int rx_buf_size;
  unsigned int max_mtu;

  /* On the Linux platform, the MTU size does not include the length
   * of the MAC address and the length of the Type, but FXGMAC_JUMBO_PACKET_MTU
   * include them.
   */
  ...
  rx_buf_size = clamp_val(rx_buf_size, FXGMAC_RX_MIN_BUF_SIZE,
                          PAGE_SIZE * 4 /* follow yonggang's suggestion */);

  rx_buf_size =
      (rx_buf_size + FXGMAC_RX_BUF_ALIGN - 1) & ~(FXGMAC_RX_BUF_ALIGN - 1);
...
static irqreturn_t fxgmac_isr(int irq, void *data) {
  ...
  for (i = 0; i < pdata->channel_count; i++) {
    ...
    if (FXGMAC_GET_REG_BITS(dma_ch_isr, DMA_CH_SR_TPS_POS, DMA_CH_SR_TPS_LEN))
      pdata->stats.tx_process_stopped++;

    if (FXGMAC_GET_REG_BITS(dma_ch_isr, DMA_CH_SR_RPS_POS, DMA_CH_SR_RPS_LEN))
      pdata->stats.rx_process_stopped++;

    if (FXGMAC_GET_REG_BITS(dma_ch_isr, DMA_CH_SR_TBU_POS, DMA_CH_SR_TBU_LEN))
      pdata->stats.tx_buffer_unavailable++;

    if (FXGMAC_GET_REG_BITS(dma_ch_isr, DMA_CH_SR_RBU_POS, DMA_CH_SR_RBU_LEN))
      pdata->stats.rx_buffer_unavailable++;

    /* Restart the device on a Fatal Bus Error */
    if (FXGMAC_GET_REG_BITS(dma_ch_isr, DMA_CH_SR_FBE_POS, DMA_CH_SR_FBE_LEN)) {
...
#endif
{
...
#else
  hw_reg_cur =
      readl(FXGMAC_DMA_REG(channel, 0x44 /* tx desc curr pointer reg */));
#endif
  if (hw_reg_cur == channel->expansion.tx_hang_hw_cur) {
    ...
    if (!pdata->tx_hang_restart_queuing) {
      ...
      DPRINTK(
          "tx_hang_timer_handler: restart scheduled, at desc %u, queuing=%u.\n",
...
static void fxgmac_tx_hang_timer_start(struct fxgmac_channel *channel) {
  ...
  if (1 && !channel->expansion.tx_hang_timer_active) {
    ...
    /* FXGMAC_INIT_DMA_TX_USECS is desc3 polling period, we give 2 more checking
     * period */
...
static void fxgmac_napi_enable(struct fxgmac_pdata *pdata, unsigned int add) {
  ...
  if (pdata->per_channel_irq) {
    channel = pdata->channel_head;
    for (i = 0; i < pdata->channel_count; i++, channel++) {
      if (!FXGMAC_GET_REG_BITS(rx_napi, i,
     ...
     RHEL_RELEASE_CODE >= RHEL_RELEASE_VERSION(9, 2))
        ...
        pdata->expansion.int_flags = FXGMAC_SET_REG_BITS(
            pdata->expansion.int_flags, FXGMAC_FLAG_RX_NAPI_FREE_POS + i,
            FXGMAC_FLAG_PER_CHAN_RX_NAPI_FREE_LEN, FXGMAC_NAPI_ENABLE);
      }
     ...
     RHEL_RELEASE_CODE >= RHEL_RELEASE_VERSION(9, 2))
        ...
        pdata->expansion.int_flags = FXGMAC_SET_REG_BITS(
            pdata->expansion.int_flags, FXGMAC_FLAG_TX_NAPI_FREE_POS,
            FXGMAC_FLAG_TX_NAPI_FREE_LEN, FXGMAC_NAPI_ENABLE);
      }
    ...
    if (!i) {
     ...
     RHEL_RELEASE_CODE >= RHEL_RELEASE_VERSION(9, 2))
...
#else
        netif_napi_add(pdata->netdev, &pdata->expansion.napi, fxgmac_all_poll,
                       NAPI_POLL_WEIGHT);
#endif
      }
      ...
      pdata->expansion.int_flags = FXGMAC_SET_REG_BITS(
          pdata->expansion.int_flags, FXGMAC_FLAG_LEGACY_NAPI_FREE_POS,
    ...
    if (channel != NULL) {
      for (i = 0; i < pdata->channel_count; i++, channel++) {
        if (FXGMAC_GET_REG_BITS(rx_napi, i,
          ...
          pdata->expansion.int_flags = FXGMAC_SET_REG_BITS(
              pdata->expansion.int_flags, FXGMAC_FLAG_RX_NAPI_FREE_POS + i,
              FXGMAC_FLAG_PER_CHAN_RX_NAPI_FREE_LEN, FXGMAC_NAPI_DISABLE);
        }
        ...
        if (FXGMAC_IS_CHANNEL_WITH_TX_IRQ(i) && tx_napi) {
          ...
          pdata->expansion.int_flags = FXGMAC_SET_REG_BITS(
              pdata->expansion.int_flags, FXGMAC_FLAG_TX_NAPI_FREE_POS,
              FXGMAC_FLAG_TX_NAPI_FREE_LEN, FXGMAC_NAPI_DISABLE);
        }
        if (netif_msg_drv(pdata)) {
      ...
      if (misc_napi) {
        ...
        pdata->expansion.int_flags = FXGMAC_SET_REG_BITS(
            pdata->expansion.int_flags, FXGMAC_FLAG_MISC_NAPI_FREE_POS,
            FXGMAC_FLAG_MISC_NAPI_FREE_LEN, FXGMAC_NAPI_DISABLE);
      }
    }
  } else {
    i = FXGMAC_GET_REG_BITS(pdata->expansion.int_flags,
...
static int fxgmac_request_irqs(struct fxgmac_pdata *pdata) {
  ...
  msi = FXGMAC_GET_REG_BITS(pdata->expansion.int_flags, FXGMAC_FLAG_MSI_POS,
                            FXGMAC_FLAG_MSI_LEN);

  msix = FXGMAC_GET_REG_BITS(pdata->expansion.int_flags, FXGMAC_FLAG_MSIX_POS,
  ...
  if (!msix) {
    if (!need_free) {
      ret = devm_request_irq(pdata->dev, pdata->dev_irq, fxgmac_isr,
                             msi ? 0 : IRQF_SHARED, netdev->name, pdata);
      if (ret) {
      ...
      pdata->expansion.int_flags = FXGMAC_SET_REG_BITS(
          pdata->expansion.int_flags, FXGMAC_FLAG_LEGACY_IRQ_FREE_POS,
  ...
  for (i = 0; i < pdata->channel_count; i++, channel++) {
    ...
    if (FXGMAC_IS_CHANNEL_WITH_TX_IRQ(i) && !tx) {
      ...
      pdata->expansion.int_flags = FXGMAC_SET_REG_BITS(
          pdata->expansion.int_flags, FXGMAC_FLAG_TX_IRQ_FREE_POS,
    ...
    if (!FXGMAC_GET_REG_BITS(rx, i, FXGMAC_FLAG_PER_CHAN_RX_IRQ_FREE_LEN)) {
      ret = devm_request_irq(pdata->dev, channel->dma_irq, fxgmac_dma_isr, 0,
      ...
      pdata->expansion.int_flags = FXGMAC_SET_REG_BITS(
          pdata->expansion.int_flags, FXGMAC_FLAG_RX_IRQ_FREE_POS + i,
          FXGMAC_FLAG_PER_CHAN_RX_IRQ_FREE_LEN, FXGMAC_IRQ_ENABLE);
    }
  }
  ...
  if (!misc) {
    snprintf(pdata->expansion.misc_irq_name,
             ...
             netdev_name(netdev));
    ret =
        devm_request_irq(pdata->dev, pdata->expansion.misc_irq, fxgmac_misc_isr,
                         0, pdata->expansion.misc_irq_name, pdata);
    if (ret) {
      netdev_alert(netdev, "error requesting misc irq %d, ret = %d\n",
                   pdata->expansion.misc_irq, ret);
      goto err_irq;
    }
    pdata->expansion.int_flags = FXGMAC_SET_REG_BITS(
        pdata->expansion.int_flags, FXGMAC_FLAG_MISC_IRQ_FREE_POS,
        FXGMAC_FLAG_MISC_IRQ_FREE_LEN, FXGMAC_IRQ_ENABLE);
  }

  if (netif_msg_drv(pdata)) {
    DPRINTK("fxgmac_req_irqs, MSIx irq request ok, total=%d,%d~%d\n", i,
            (pdata->channel_head)[0].dma_irq,
  ...
  if (pdata->per_channel_irq) {
    for (i--, channel--; i < pdata->channel_count; i--, channel--) {
      if (FXGMAC_IS_CHANNEL_WITH_TX_IRQ(i) && tx) {
        pdata->expansion.int_flags = FXGMAC_SET_REG_BITS(
            pdata->expansion.int_flags, FXGMAC_FLAG_TX_IRQ_FREE_POS,
            FXGMAC_FLAG_TX_IRQ_FREE_LEN, FXGMAC_IRQ_DISABLE);
      ...
      if (FXGMAC_GET_REG_BITS(rx, i, FXGMAC_FLAG_PER_CHAN_RX_IRQ_FREE_LEN)) {
        pdata->expansion.int_flags = FXGMAC_SET_REG_BITS(
            pdata->expansion.int_flags, FXGMAC_FLAG_RX_IRQ_FREE_POS + i,
            FXGMAC_FLAG_PER_CHAN_RX_IRQ_FREE_LEN, FXGMAC_IRQ_DISABLE);
    ...
    if (misc) {
      pdata->expansion.int_flags = FXGMAC_SET_REG_BITS(
          pdata->expansion.int_flags, FXGMAC_FLAG_MISC_IRQ_FREE_POS,
          FXGMAC_FLAG_MISC_IRQ_FREE_LEN, FXGMAC_IRQ_DISABLE);
...
static void fxgmac_free_irqs(struct fxgmac_pdata *pdata) {
  ...
  if (!msix) {
    if (need_free) {
      ...
      pdata->expansion.int_flags = FXGMAC_SET_REG_BITS(
          pdata->expansion.int_flags, FXGMAC_FLAG_LEGACY_IRQ_FREE_POS,
          FXGMAC_FLAG_LEGACY_IRQ_FREE_LEN, FXGMAC_IRQ_DISABLE);
    }
  }
...
static int fxgmac_disable_pci_msi_config(struct pci_dev *pdev) {
  ...
  pcie_msi_mask_bits =
      FXGMAC_SET_REG_BITS(pcie_msi_mask_bits, PCI_CAP_ID_MSI_ENABLE_POS,
...
static int fxgmac_disable_pci_msix_config(struct pci_dev *pdev) {
  ...
  pcie_msi_mask_bits =
      FXGMAC_SET_REG_BITS(pcie_msi_mask_bits, PCI_CAP_ID_MSIX_ENABLE_POS,
...
int fxgmac_start(struct fxgmac_pdata *pdata) {
  ...
  if (FXGMAC_GET_REG_BITS(pdata->expansion.int_flags, FXGMAC_FLAG_LEGACY_POS,
                          FXGMAC_FLAG_LEGACY_LEN)) {
    /*
     * we should disable msi and msix here when we use legacy interrupt,for two
     * reasons:
     ...
     *   will be turned on again by default after waking up from S4 on some
...
void fxgmac_net_powerdown(struct fxgmac_pdata *pdata, unsigned int wol) {
  ...
  /* phy polling timer should detect the state of fxgmac and stop link status
   * polling accordingly */
...
static netdev_tx_t fxgmac_xmit(struct sk_buff *skb, struct net_device *netdev) {
  ...
  ret = fxgmac_maybe_stop_tx_queue(channel, ring, tx_pkt_info->desc_count);
  if (ret) {
    return ret;
  }
...
static int fxgmac_ioctl(struct net_device *netdev, struct ifreq *ifr, int cmd) {
  ...
  if (!netif_running(netdev) || pdata->expansion.dev_state != FXGMAC_DEV_START)
    return -ENODEV;
...
static int fxgmac_set_features(struct net_device *netdev,
  ...
  rxvlan_filter =
      pdata->expansion.netdev_features & NETIF_F_HW_VLAN_CTAG_FILTER;
...
#define napi_alloc_skb(a, b) NULL
  ...
  dma_sync_single_range_for_cpu(pdata->dev, desc_data->rx.hdr.dma_base,
                                ...
                                desc_data->rx.hdr.dma_len, DMA_FROM_DEVICE);
...
static int fxgmac_tx_poll(struct fxgmac_channel *channel) {
  ...
  if ((ring->cur != ring->dirty) && (netif_msg_tx_done(pdata))) {
    DPRINTK("tx_poll callin, ring_cur=%d,ring_dirty=%d,qIdx=%d\n", ring->cur,
            ring->dirty, channel->queue_index);
  }
...
#define FXGMAC_HANG_THRESHOLD 1
      ...
      if (change_cnt > 2) {
        ...
        DPRINTK("after complete check, cur=%d, dirty=%d,qIdx=%d, hw desc "
                "cur=%#x, pre=%#x\n",
                ring->cur, ring->dirty, channel->queue_index, reg_cur,
                reg_cur_pre);

        if ((ring->cur > ring->dirty) &&
            ((ring->cur - ring->dirty) > FXGMAC_HANG_THRESHOLD)) {
          DPRINTK("after complete check warning..., too many TBD occupied by "
                  "HW, 0xdbbb, %d.\n",
                  (ring->cur - ring->dirty));
...
#else
          channel->expansion.tx_hang_hw_cur = readl(
              FXGMAC_DMA_REG(channel, 0x44 /* tx desc curr pointer reg */));
#endif
          /* double check for race conditione */
          if ((!pdata->tx_hang_restart_queuing) &&
              (!channel->expansion.tx_hang_timer_active)) {
            DPRINTK("tx_hang polling: start timer at desc %u, timer act=%u, "
                    "queuing=%u, qidx=%u.\n",
                    reg_cur, channel->expansion.tx_hang_timer_active,
                    pdata->tx_hang_restart_queuing, channel->queue_index);
            fxgmac_tx_hang_timer_start(channel);
          }
        }
      } else if (pdata->tx_hang_restart_queuing) {
        // if (netif_msg_drv(pdata)) {
        DPRINTK("tx_hang_timer_handler: restart scheduled.\n");
      }
    }
#endif
...
static int fxgmac_rx_poll(struct fxgmac_channel *channel, int budget) {
  ...
  read_again:
    ...
    incomplete = FXGMAC_GET_REG_BITS(pkt_info->attributes,
                                     RX_PACKET_ATTRIBUTES_INCOMPLETE_POS,
                                     RX_PACKET_ATTRIBUTES_INCOMPLETE_LEN);
    context_next = FXGMAC_GET_REG_BITS(pkt_info->attributes,
    ...
    if (!context) {
      ...
      if (len > max_len) {
        if (net_ratelimit())
          netif_err(pdata, rx_err, netdev, "len %d larger than max size %d\n",
                    len, max_len);
        pdata->netdev->stats.rx_length_errors++;
        pdata->netdev->stats.rx_dropped++;
        dev_kfree_skb(skb);
    ...
    if (FXGMAC_GET_REG_BITS(pkt_info->attributes,
      ...
      __vlan_hwaccel_put_tag(skb, htons(ETH_P_8021Q), pkt_info->vlan_ctag);
    ...
    if (FXGMAC_GET_REG_BITS(pkt_info->attributes,
      ...
      skb_set_hash(skb, pkt_info->rss_hash, pkt_info->rss_hash_type);
    ...
    if (pdata->expansion.fxgmac_test_tso_flag) {
      ...
      if (pdata->expansion.fxgmac_test_tso_seg_num == 1) {
        ...
        if (pdata->expansion.fxgmac_test_last_tso_len ==
          ...
          /* process packet */
          if ((pdata->expansion.fxgmac_test_skb_arr_in_index + 1) %
            ...
            pdata->expansion.fxgmac_test_skb_array
                [pdata->expansion.fxgmac_test_skb_arr_in_index] = tmpskb;
            pdata->expansion.fxgmac_test_skb_arr_in_index =
                (pdata->expansion.fxgmac_test_skb_arr_in_index + 1) %
                FXGMAC_MAX_DBG_TEST_PKT;
          } else {
...
static int fxgmac_one_poll_rx(struct napi_struct *napi, int budget) {
  ...
  if (processed < budget) {
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 10, 0))
    /* if there no interrupt occurred when this interrupt running,struct napi's
     * state is NAPIF_STATE_SCHED, napi_complete_done return true and we can
     * enable irq,it will not cause unbalanced iqr issure. if there more
     * interrupt occurred when this interrupt running,struct napi's state is
     * NAPIF_STATE_SCHED | NAPIF_STATE_MISSED because napi_schedule_prep will
     * make it. At this time napi_complete_done will return false and schedule
     * poll again because of NAPIF_STATE_MISSED,it will cause unbalanced irq
     * issure.
     */
...
void fxgmac_dev_xmit(struct fxgmac_channel *channel) {
  ...
  if (tso_context || vlan_context) {
    if (tso_context) {
      ...
      /* Set the MSS size */
      dma_desc->desc2 =
          FXGMAC_SET_REG_BITS_LE(dma_desc->desc2, TX_CONTEXT_DESC2_MSS_POS,
                                 TX_CONTEXT_DESC2_MSS_LEN, pkt_info->mss);

      /* Mark it as a CONTEXT descriptor */
      dma_desc->desc3 =
          FXGMAC_SET_REG_BITS_LE(dma_desc->desc3, TX_CONTEXT_DESC3_CTXT_POS,
                                 TX_CONTEXT_DESC3_CTXT_LEN, 1);

      /* Indicate this descriptor contains the MSS */
      dma_desc->desc3 =
          FXGMAC_SET_REG_BITS_LE(dma_desc->desc3, TX_CONTEXT_DESC3_TCMSSV_POS,
    ...
    if (vlan_context) {
      ...
      /* Set the VLAN tag */
      dma_desc->desc3 =
          FXGMAC_SET_REG_BITS_LE(dma_desc->desc3, TX_CONTEXT_DESC3_VT_POS,
                                 TX_CONTEXT_DESC3_VT_LEN, pkt_info->vlan_ctag);

      /* Indicate this descriptor contains the VLAN tag */
      dma_desc->desc3 =
          FXGMAC_SET_REG_BITS_LE(dma_desc->desc3, TX_CONTEXT_DESC3_VLTV_POS,
  ...
  dma_desc->desc2 = FXGMAC_SET_REG_BITS_LE(
      dma_desc->desc2, TX_NORMAL_DESC2_HL_B1L_POS, TX_NORMAL_DESC2_HL_B1L_LEN,
      desc_data->skb_dma_len);

  /* VLAN tag insertion check */
  if (vlan) {
    dma_desc->desc2 = FXGMAC_SET_REG_BITS_LE(
        dma_desc->desc2, TX_NORMAL_DESC2_VTIR_POS, TX_NORMAL_DESC2_VTIR_LEN,
        TX_NORMAL_DESC2_VLAN_INSERT);
    pdata->stats.tx_vlan_packets++;
  }

  /* Timestamp enablement check */
  if (FXGMAC_GET_REG_BITS(pkt_info->attributes, TX_PACKET_ATTRIBUTES_PTP_POS,
                          TX_PACKET_ATTRIBUTES_PTP_LEN))
    dma_desc->desc2 = FXGMAC_SET_REG_BITS_LE(
        dma_desc->desc2, TX_NORMAL_DESC2_TTSE_POS, TX_NORMAL_DESC2_TTSE_LEN, 1);

  /* Mark it as First Descriptor */
  dma_desc->desc3 = FXGMAC_SET_REG_BITS_LE(
      dma_desc->desc3, TX_NORMAL_DESC3_FD_POS, TX_NORMAL_DESC3_FD_LEN, 1);

  /* Mark it as a NORMAL descriptor */
  dma_desc->desc3 = FXGMAC_SET_REG_BITS_LE(
      dma_desc->desc3, TX_NORMAL_DESC3_CTXT_POS, TX_NORMAL_DESC3_CTXT_LEN, 0);

  /* Set OWN bit if not the first descriptor */
  if (cur_index != start_index)
    dma_desc->desc3 = FXGMAC_SET_REG_BITS_LE(
        dma_desc->desc3, TX_NORMAL_DESC3_OWN_POS, TX_NORMAL_DESC3_OWN_LEN, 1);

  if (tso) {
    /* Enable TSO */
    dma_desc->desc3 = FXGMAC_SET_REG_BITS_LE(
        dma_desc->desc3, TX_NORMAL_DESC3_TSE_POS, TX_NORMAL_DESC3_TSE_LEN, 1);
    dma_desc->desc3 = FXGMAC_SET_REG_BITS_LE(
        dma_desc->desc3, TX_NORMAL_DESC3_TCPPL_POS, TX_NORMAL_DESC3_TCPPL_LEN,
        pkt_info->tcp_payload_len);
    dma_desc->desc3 = FXGMAC_SET_REG_BITS_LE(
        dma_desc->desc3, TX_NORMAL_DESC3_TCPHDRLEN_POS,
        TX_NORMAL_DESC3_TCPHDRLEN_LEN, pkt_info->tcp_header_len / 4);

    pdata->stats.tx_tso_packets++;
  } else {
    /* Enable CRC and Pad Insertion */
    dma_desc->desc3 = FXGMAC_SET_REG_BITS_LE(
        dma_desc->desc3, TX_NORMAL_DESC3_CPC_POS, TX_NORMAL_DESC3_CPC_LEN, 0);

    /* Enable HW CSUM */
    if (csum)
      dma_desc->desc3 =
          FXGMAC_SET_REG_BITS_LE(dma_desc->desc3, TX_NORMAL_DESC3_CIC_POS,
                                 TX_NORMAL_DESC3_CIC_LEN, 0x3);

    /* Set the total length to be transmitted */
    dma_desc->desc3 =
        FXGMAC_SET_REG_BITS_LE(dma_desc->desc3, TX_NORMAL_DESC3_FL_POS,
                               TX_NORMAL_DESC3_FL_LEN, pkt_info->length);
  }
  if (netif_msg_drv(pdata)) {
    DPRINTK("dev_xmit before more descs, desc cur=%d, start=%d, "
            "desc=%#x,%#x,%#x,%#x\n",
            cur_index, start_index, dma_desc->desc0, dma_desc->desc1,
  ...
  dma_desc->desc3 = FXGMAC_SET_REG_BITS_LE(
      dma_desc->desc3, TX_NORMAL_DESC3_LD_POS, TX_NORMAL_DESC3_LD_LEN, 1);

  dma_desc->desc2 = FXGMAC_SET_REG_BITS_LE(
      dma_desc->desc2, TX_NORMAL_DESC2_IC_POS, TX_NORMAL_DESC2_IC_LEN, 1);
  ...
  if (netif_msg_drv(pdata)) {
    DPRINTK("dev_xmit last descs, desc cur=%d, desc=%#x,%#x,%#x,%#x\n",
            cur_index, dma_desc->desc0, dma_desc->desc1, dma_desc->desc2,
            dma_desc->desc3);
  }
  ...
  if (netif_msg_drv(pdata)) {
    DPRINTK("dev_xmit first descs, start=%d, desc=%#x,%#x,%#x,%#x\n",
            start_index, dma_desc->desc0, dma_desc->desc1, dma_desc->desc2,
            dma_desc->desc3);
  }

  if (netif_msg_tx_queued(pdata))
    fxgmac_dump_tx_desc(pdata, ring, start_index, pkt_info->desc_count, 1);
...
static void fxgmac_get_rx_tstamp(struct fxgmac_pkt_info *pkt_info,
  ...
  if (nsec != 0xffffffffffffffffULL) {
    ...
    pkt_info->attributes = FXGMAC_SET_REG_BITS(
        pkt_info->attributes, RX_PACKET_ATTRIBUTES_RX_TSTAMP_POS,
...
static int fxgmac_dev_read(struct fxgmac_channel *channel) {
  ...
  if (FXGMAC_GET_REG_BITS_LE(dma_desc->desc3, RX_NORMAL_DESC3_CTXT_POS,
    ...
    pkt_info->attributes = FXGMAC_SET_REG_BITS(
        pkt_info->attributes, RX_PACKET_ATTRIBUTES_CONTEXT_NEXT_POS,
  ...
  if (FXGMAC_GET_REG_BITS_LE(dma_desc->desc3, RX_NORMAL_DESC3_FD_POS,
    ...
    desc_data->rx.hdr_len = FXGMAC_GET_REG_BITS_LE(
        dma_desc->desc2, RX_NORMAL_DESC2_HL_POS, RX_NORMAL_DESC2_HL_LEN);
  ...
  desc_data->rx.len = FXGMAC_GET_REG_BITS_LE(
      dma_desc->desc3, RX_NORMAL_DESC3_PL_POS, RX_NORMAL_DESC3_PL_LEN);
  ...
  if (err || rxparser == 0x7) {
    pkt_info->errors =
        FXGMAC_SET_REG_BITS(pkt_info->errors, RX_PACKET_ERRORS_FRAME_POS,
  ...
  if ((etlt == 0x4) && (netdev->features & NETIF_F_HW_VLAN_CTAG_RX)) {
    ...
    pkt_info->vlan_ctag = FXGMAC_GET_REG_BITS_LE(
        dma_desc->desc0, RX_NORMAL_DESC0_OVT_POS, RX_NORMAL_DESC0_OVT_LEN);
