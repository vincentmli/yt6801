...
static void fxgmac_set_mac_reg(struct fxgmac_pdata *pdata,
  ...
  if (ha) {
    ...
    netif_dbg(pdata, drv, pdata->netdev, "adding mac addredd at %#x\n",
              *mac_reg);

    mac_addr_hi = FXGMAC_SET_REG_BITS(mac_addr_hi, MAC_MACA1HR_AE_POS,
...
static int fxgmac_enable_tx_vlan(struct fxgmac_pdata *pdata) {
  ...
  regval =
      FXGMAC_SET_REG_BITS(regval, MAC_VLANIR_VLTI_POS, MAC_VLANIR_VLTI_LEN, 0);
  regval =
      FXGMAC_SET_REG_BITS(regval, MAC_VLANIR_CSVL_POS, MAC_VLANIR_CSVL_LEN, 0);
  regval =
      FXGMAC_SET_REG_BITS(regval, MAC_VLANIR_VLP_POS, MAC_VLANIR_VLP_LEN, 1);
  regval =
      FXGMAC_SET_REG_BITS(regval, MAC_VLANIR_VLC_POS, MAC_VLANIR_VLC_LEN, 2);
  regval = FXGMAC_SET_REG_BITS(regval, MAC_VLANIR_VLT_POS, MAC_VLANIR_VLT_LEN,
                               pdata->vlan);
  writereg(pdata->pAdapter, regval, pdata->mac_regs + MAC_VLANIR);

  regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_VLANTR);
  regval = FXGMAC_SET_REG_BITS(regval, MAC_VLANTR_VL_POS, MAC_VLANTR_VL_LEN,
                               pdata->vlan);
...
static int fxgmac_disable_tx_vlan(struct fxgmac_pdata *pdata) {
  ...
  regval =
      FXGMAC_SET_REG_BITS(regval, MAC_VLANIR_CSVL_POS, MAC_VLANIR_CSVL_LEN, 0);
  regval = FXGMAC_SET_REG_BITS(regval, MAC_VLANIR_VLTI_POS, MAC_VLANIR_VLTI_LEN,
                               /*0*/ 1);
  // Set VLAN priority control disable
  regval = FXGMAC_SET_REG_BITS(regval, MAC_VLANIR_VLP_POS, MAC_VLANIR_VLP_LEN,
                               /*1*/ 0);
  regval =
      FXGMAC_SET_REG_BITS(regval, MAC_VLANIR_VLC_POS, MAC_VLANIR_VLC_LEN, 0);
  writereg(pdata->pAdapter, regval, pdata->mac_regs + MAC_VLANIR);
  ...
  // If it enable the following code and enable vlan filter, then the MAC only
  // receive the packet of VLAN ID "1"
...
static int fxgmac_enable_rx_vlan_stripping(struct fxgmac_pdata *pdata) {
  ...
  regval =
      FXGMAC_SET_REG_BITS(regval, MAC_VLANTR_ESVL_POS, MAC_VLANTR_ESVL_LEN, 0);
  /* Enable VLAN tag stripping */
  regval = FXGMAC_SET_REG_BITS(regval, MAC_VLANTR_EVLS_POS, MAC_VLANTR_EVLS_LEN,
                               0x3);
  writereg(pdata->pAdapter, regval, pdata->mac_regs + MAC_VLANTR);
...
static int fxgmac_disable_rx_vlan_stripping(struct fxgmac_pdata *pdata) {
  ...
  regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_VLANTR);
  regval =
      FXGMAC_SET_REG_BITS(regval, MAC_VLANTR_EVLS_POS, MAC_VLANTR_EVLS_LEN, 0);
  writereg(pdata->pAdapter, regval, pdata->mac_regs + MAC_VLANTR);
...
static int fxgmac_enable_rx_vlan_filtering(struct fxgmac_pdata *pdata) {
  ...
  regval = FXGMAC_SET_REG_BITS(regval, MAC_PFR_VTFE_POS, MAC_PFR_VTFE_LEN, 1);
...
// In Linux driver it uses VLAN Filter to filter packet, In windows driver it
// uses SW method to filter packet.
  ...
  regval =
      FXGMAC_SET_REG_BITS(regval, MAC_VLANTR_VTHM_POS, MAC_VLANTR_VTHM_LEN, 1);
  /* Disable VLAN tag inverse matching */
  regval =
      FXGMAC_SET_REG_BITS(regval, MAC_VLANTR_VTIM_POS, MAC_VLANTR_VTIM_LEN, 0);
  /* Only filter on the lower 12-bits of the VLAN tag */
  regval =
      FXGMAC_SET_REG_BITS(regval, MAC_VLANTR_ETV_POS, MAC_VLANTR_ETV_LEN, 1);
#endif
...
static int fxgmac_disable_rx_vlan_filtering(struct fxgmac_pdata *pdata) {
  ...
  regval = FXGMAC_SET_REG_BITS(regval, MAC_PFR_VTFE_POS, MAC_PFR_VTFE_LEN, 0);
  writereg(pdata->pAdapter, regval, pdata->mac_regs + MAC_PFR);
...
static int fxgmac_set_promiscuous_mode(struct fxgmac_pdata *pdata,
  ...
  DbgPrintF(MP_TRACE,
            "note, vlan filter is called when set promiscuous mode=%d", enable);
...
static int fxgmac_enable_rx_broadcast(struct fxgmac_pdata *pdata,
                                      unsigned int enable) {
  unsigned int val =
      enable ? 0 : 1; // mac reg bit is disable,,so invert the val.
  u32 regval;
 ...
 * input:     pmc_mac, pointer to mcast MAC. if it is null, then clean all
 * registers. b_add, 1 to set the bit; 0 to clear the bit.
 */
static void fxgmac_config_multicast_mac_hash_table(struct fxgmac_pdata *pdata,
  ...
  if (!pmc_mac) {
    for (j = 0; j < FXGMAC_MAC_HASH_TABLE_SIZE; j++) {
      ...
      // DBGPRINT(MP_TRACE, ("fxgmac_config_mcast_mac_hash_table, reg
      // %04x=0x%x\n", FXGMAC_MAC_REGS_OFFSET + hash_reg,
      // readreg(pdata->mac_regs + hash_reg)));
    }
    DBGPRINT(MP_TRACE, ("fxgmac_config_mcast_mac_hash_table, clear all mcast "
                        "mac hash table size %d",
                        j));
    return;
  }
...
static void fxgmac_config_mac_address(struct fxgmac_pdata *pdata) {
  ...
  // fxgmac_set_mac_address(pdata, (u8*)"\x00\x55\x7b\xb5\x7d\xf7");//
  // pdata->netdev->dev_addr); fxgmac_set_mac_address(pdata,
  // (u8*)"\xf7\x7d\xb5\x7b\x55\x00");

  /* Filtering is done using perfect filtering and hash filtering */
  if (pdata->hw_feat.hash_table_size) {
    regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_PFR);
    regval = FXGMAC_SET_REG_BITS(regval, MAC_PFR_HPF_POS, MAC_PFR_HPF_LEN, 1);
#if FXGMAC_MAC_HASH_TABLE
    regval = FXGMAC_SET_REG_BITS(regval, MAC_PFR_HUC_POS, MAC_PFR_HUC_LEN, 1);
#endif
    regval = FXGMAC_SET_REG_BITS(regval, MAC_PFR_HMC_POS, MAC_PFR_HMC_LEN, 1);
    writereg(pdata->pAdapter, regval, pdata->mac_regs + MAC_PFR);
  }
}
...
static int fxgmac_config_crc_check(struct fxgmac_pdata *pdata) {
  ...
  regval =
      FXGMAC_SET_REG_BITS(regval, MAC_ECR_DCRCC_POS, MAC_ECR_DCRCC_LEN, value);
...
static int fxgmac_config_jumbo(struct fxgmac_pdata *pdata) {
  ...
  regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_CR);
  regval =
      FXGMAC_SET_REG_BITS(regval, MAC_CR_JE_POS, MAC_CR_JE_LEN, pdata->jumbo);
  writereg(pdata->pAdapter, regval, pdata->mac_regs + MAC_CR);
  return 0;
}
...
static void fxgmac_config_vlan_support(struct fxgmac_pdata *pdata) {
  ...
  // Windows driver set vlan_filter disable, but in Linux driver it enable
  // vlan_filter
...
static void fxgmac_prepare_tx_stop(struct fxgmac_pdata *pdata,
  ...
  if (channel->queue_index < DMA_DSRX_FIRST_QUEUE) {
    tx_dsr = DMA_DSR0;
    tx_pos = (channel->queue_index * DMA_DSR_Q_LEN) + DMA_DSR0_TPS_START;
  } else {
    tx_qidx = channel->queue_index - DMA_DSRX_FIRST_QUEUE;

    tx_dsr = DMA_DSR1 + ((tx_qidx / DMA_DSRX_QPR) * DMA_DSRX_INC);
    tx_pos = ((tx_qidx % DMA_DSRX_QPR) * DMA_DSR_Q_LEN) + DMA_DSRX_TPS_START;
  }
  // 2022-04-19 xiaojiang comment
  ...
  while (time_before(jiffies, tx_timeout)) {
    ...
    tx_status = FXGMAC_GET_REG_BITS(tx_status, tx_pos, DMA_DSR_TPS_LEN);
...
static void fxgmac_enable_tx(struct fxgmac_pdata *pdata) {
  ...
  for (i = 0; i < pdata->channel_count; i++, channel++) {
    ...
    regval =
        FXGMAC_SET_REG_BITS(regval, DMA_CH_TCR_ST_POS, DMA_CH_TCR_ST_LEN, 1);
...
static void fxgmac_disable_tx(struct fxgmac_pdata *pdata) {
  ...
  if (channel != NULL) {
    for (i = 0; i < pdata->channel_count; i++, channel++) {
      ...
      regval = readreg(pdata->pAdapter, FXGMAC_DMA_REG(channel, DMA_CH_TCR));
      regval =
          FXGMAC_SET_REG_BITS(regval, DMA_CH_TCR_ST_POS, DMA_CH_TCR_ST_LEN, 0);
      writereg(pdata->pAdapter, regval, FXGMAC_DMA_REG(channel, DMA_CH_TCR));
    }
  }
#else
...
static void fxgmac_prepare_rx_stop(struct fxgmac_pdata *pdata,
  ...
  while (time_before(jiffies, rx_timeout)) {
    rx_status =
        readreg(pdata->pAdapter, FXGMAC_MTL_REG(pdata, queue, MTL_Q_RQDR));
  ...
  if (!time_before(jiffies, rx_timeout))
    netdev_info(pdata->netdev, "timed out waiting for Rx queue %u to empty\n",
                queue);
#else
  ...
  if (0 == busy) {
    rx_status =
        readreg(pdata->pAdapter, FXGMAC_MTL_REG(pdata, queue, MTL_Q_RQDR));
    DbgPrintF(MP_WARN,
              "warning !!!timed out waiting for Rx queue %u to empty\n", queue);
  }
#endif
}
...
static void fxgmac_enable_rx(struct fxgmac_pdata *pdata) {
  ...
  for (i = 0; i < pdata->channel_count; i++, channel++) {
    ...
    regval =
        FXGMAC_SET_REG_BITS(regval, DMA_CH_RCR_SR_POS, DMA_CH_RCR_SR_LEN, 1);
...
static void fxgmac_enable_channel_rx(struct fxgmac_pdata *pdata,
  ...
  if (!channel->rx_ring)
    return;
  regval = readreg(pdata->pAdapter, FXGMAC_DMA_REG(channel, DMA_CH_RCR));
  regval = FXGMAC_SET_REG_BITS(regval, DMA_CH_RCR_SR_POS, DMA_CH_RCR_SR_LEN, 1);
  writereg(pdata->pAdapter, regval, FXGMAC_DMA_REG(channel, DMA_CH_RCR));
  /* Enable  Rx queue */
...
static void fxgmac_disable_rx(struct fxgmac_pdata *pdata) {
  ...
  if (channel != NULL) {
    for (i = 0; i < pdata->channel_count; i++, channel++) {
      ...
      regval = readreg(pdata->pAdapter, FXGMAC_DMA_REG(channel, DMA_CH_RCR));
      regval =
          FXGMAC_SET_REG_BITS(regval, DMA_CH_RCR_SR_POS, DMA_CH_RCR_SR_LEN, 0);
      writereg(pdata->pAdapter, regval, FXGMAC_DMA_REG(channel, DMA_CH_RCR));
    }
  }
#else
...
static int fxgmac_is_context_desc(struct fxgmac_dma_desc *dma_desc) {
  ...
  int regval;
  regval = (int)FXGMAC_GET_REG_BITS_LE(
      dma_desc->desc3, TX_NORMAL_DESC3_CTXT_POS, TX_NORMAL_DESC3_CTXT_LEN);
  return regval;
}
...
static int fxgmac_is_last_desc(struct fxgmac_dma_desc *dma_desc) {
  ...
  int regval;
  regval = (int)FXGMAC_GET_REG_BITS_LE(dma_desc->desc3, TX_NORMAL_DESC3_LD_POS,
...
static int fxgmac_disable_tx_flow_control(struct fxgmac_pdata *pdata) {
  ...
  for (i = 0; i < q_count; i++) {
    ...
    regval =
        FXGMAC_SET_REG_BITS(regval, MAC_Q0TFCR_TFE_POS, MAC_Q0TFCR_TFE_LEN, 0);
...
static int fxgmac_enable_tx_flow_control(struct fxgmac_pdata *pdata) {
  ...
  for (i = 0; i < q_count; i++) {
    ...
    regval =
        FXGMAC_SET_REG_BITS(regval, MAC_Q0TFCR_TFE_POS, MAC_Q0TFCR_TFE_LEN, 1);
    /* Set pause time */
    regval = FXGMAC_SET_REG_BITS(regval, MAC_Q0TFCR_PT_POS, MAC_Q0TFCR_PT_LEN,
                                 0xffff);
...
static int fxgmac_disable_rx_flow_control(struct fxgmac_pdata *pdata) {
  ...
  regval = FXGMAC_SET_REG_BITS(regval, MAC_RFCR_RFE_POS, MAC_RFCR_RFE_LEN, 0);
...
static int fxgmac_enable_rx_flow_control(struct fxgmac_pdata *pdata) {
  ...
  regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_RFCR);
  regval = FXGMAC_SET_REG_BITS(regval, MAC_RFCR_RFE_POS, MAC_RFCR_RFE_LEN, 1);
  writereg(pdata->pAdapter, regval, pdata->mac_regs + MAC_RFCR);
...
static int fxgmac_config_rx_coalesce(struct fxgmac_pdata *pdata) {
  ...
  for (i = 0; i < pdata->channel_count; i++, channel++) {
    ...
    regval = FXGMAC_SET_REG_BITS(regval, DMA_CH_RIWT_RWT_POS,
                                 DMA_CH_RIWT_RWT_LEN, pdata->rx_riwt);
...
static void fxgmac_config_rx_fep_disable(struct fxgmac_pdata *pdata) {
  ...
  for (i = 0; i < pdata->rx_q_count; i++) {
    ...
    regval = FXGMAC_SET_REG_BITS(
        regval, MTL_Q_RQOMR_FEP_POS, MTL_Q_RQOMR_FEP_LEN,
        MTL_FEP_ENABLE); // 1:enable  the  rx queue forward packet with error
                         // status(crc error,gmii_er,watch dog timeout.or
                         // overflow)
    writereg(pdata->pAdapter, regval, FXGMAC_MTL_REG(pdata, i, MTL_Q_RQOMR));
  }
}
...
static void fxgmac_config_rx_buffer_size(struct fxgmac_pdata *pdata) {
  ...
  for (i = 0; i < pdata->channel_count; i++, channel++) {
    ...
    regval = FXGMAC_SET_REG_BITS(regval, DMA_CH_RCR_RBSZ_POS,
                                 DMA_CH_RCR_RBSZ_LEN, pdata->rx_buf_size);
    writereg(pdata->pAdapter, regval, FXGMAC_DMA_REG(channel, DMA_CH_RCR));
  }
#else
  struct fxgmac_rx_queue *rxq;
  ...
  for (i = 0; i < pdata->expansion.eth_dev->data->nb_rx_queues; i++) {
    ...
    rxq->buf_size =
        (rxq->buf_size + FXGMAC_RX_BUF_ALIGN - 1) & ~(FXGMAC_RX_BUF_ALIGN - 1);
...
static void fxgmac_config_tso_mode(struct fxgmac_pdata *pdata) {
  ...
  for (i = 0; i < pdata->channel_count; i++, channel++) {
    ...
    regval = readreg(pdata->pAdapter, FXGMAC_DMA_REG(channel, DMA_CH_TCR));
    regval = FXGMAC_SET_REG_BITS(regval, DMA_CH_TCR_TSE_POS, DMA_CH_TCR_TSE_LEN,
                                 tso);
    writereg(pdata->pAdapter, regval, FXGMAC_DMA_REG(channel, DMA_CH_TCR));
  }
#else
...
static void fxgmac_config_sph_mode(struct fxgmac_pdata *pdata) {
  ...
  for (i = 0; i < pdata->channel_count; i++, channel++) {
    ...
    regval =
        FXGMAC_SET_REG_BITS(regval, DMA_CH_CR_SPH_POS, DMA_CH_CR_SPH_LEN, 0);
  ...
  regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_ECR);
  regval = FXGMAC_SET_REG_BITS(regval, MAC_ECR_HDSMS_POS, MAC_ECR_HDSMS_LEN,
...
static void fxgmac_config_mtl_mode(struct fxgmac_pdata *pdata) {
  ...
  regval = FXGMAC_SET_REG_BITS(regval, MTL_OMR_ETSALG_POS, MTL_OMR_ETSALG_LEN,
                               MTL_ETSALG_WRR);
  ...
  for (i = 0; i < pdata->tx_q_count /*hw_feat.tc_cnt*/; i++) {
    ...
    regval =
        FXGMAC_SET_REG_BITS(regval, MTL_TC_QWR_QW_POS, MTL_TC_QWR_QW_LEN, 1);
  ...
  regval = readreg(pdata->pAdapter, pdata->mac_regs + MTL_OMR);
  regval =
      FXGMAC_SET_REG_BITS(regval, MTL_OMR_RAA_POS, MTL_OMR_RAA_LEN, MTL_RAA_SP);
  writereg(pdata->pAdapter, regval, pdata->mac_regs + MTL_OMR);
}
...
static void fxgmac_config_queue_mapping(struct fxgmac_pdata *pdata) {
  ...
  prio_queues = min_t(unsigned int, IEEE_8021QAZ_MAX_TCS, pdata->rx_q_count);
  ...
  for (i = 0, prio = 0; i < prio_queues;) {
    ...
    if (i < ppq_extra) {
      ...
      netif_dbg(pdata, drv, pdata->netdev, " mapped to Rxq%u", i);
      mask |= (1 << prio);
      prio++;
    }
  ...
  if (pdata->rss) {
    /* in version later 0617, need to enable DA-based DMA Channel Selection to
     * let RSS work, ie, bit4,12,20,28 for Q0,1,2,3 individual
     */
...
static u32 fxgmac_calculate_max_checksum_size(struct fxgmac_pdata *pdata) {
   ...
   */
  fifo_size -=
      (pdata->tx_pbl * (pdata->pblx8 ? 8 : 1) + 5) * (FXGMAC_DATA_WIDTH / 8);
  fifo_size -= 256;
...
static int fxgmac_config_osp_mode(struct fxgmac_pdata *pdata) {
  ...
  for (i = 0; i < pdata->channel_count; i++, channel++) {
    ...
    regval = readreg(pdata->pAdapter, FXGMAC_DMA_REG(channel, DMA_CH_TCR));
    regval = FXGMAC_SET_REG_BITS(regval, DMA_CH_TCR_OSP_POS, DMA_CH_TCR_OSP_LEN,
                                 pdata->tx_osp_mode);
    writereg(pdata->pAdapter, regval, FXGMAC_DMA_REG(channel, DMA_CH_TCR));
  }
#else
...
static u32 fxgmac_get_tx_pbl_val(struct fxgmac_pdata *pdata) {
  ...
  regval =
      readreg(pdata->pAdapter, FXGMAC_DMA_REG(pdata->channel_head, DMA_CH_TCR));
  regval = FXGMAC_GET_REG_BITS(regval, DMA_CH_TCR_PBL_POS, DMA_CH_TCR_PBL_LEN);
  return regval;
}
...
static int fxgmac_config_tx_pbl_val(struct fxgmac_pdata *pdata) {
  ...
  for (i = 0; i < pdata->channel_count; i++, channel++) {
    ...
    regval = readreg(pdata->pAdapter, FXGMAC_DMA_REG(channel, DMA_CH_TCR));
    regval = FXGMAC_SET_REG_BITS(regval, DMA_CH_TCR_PBL_POS, DMA_CH_TCR_PBL_LEN,
                                 pdata->tx_pbl);
    writereg(pdata->pAdapter, regval, FXGMAC_DMA_REG(channel, DMA_CH_TCR));
  }
#else
...
static u32 fxgmac_get_rx_pbl_val(struct fxgmac_pdata *pdata) {
  ...
  regval =
      readreg(pdata->pAdapter, FXGMAC_DMA_REG(pdata->channel_head, DMA_CH_RCR));
  regval = FXGMAC_GET_REG_BITS(regval, DMA_CH_RCR_PBL_POS, DMA_CH_RCR_PBL_LEN);
  return regval;
}
...
static int fxgmac_config_rx_pbl_val(struct fxgmac_pdata *pdata) {
  ...
  for (i = 0; i < pdata->channel_count; i++, channel++) {
    ...
    regval = readreg(pdata->pAdapter, FXGMAC_DMA_REG(channel, DMA_CH_RCR));
    regval = FXGMAC_SET_REG_BITS(regval, DMA_CH_RCR_PBL_POS, DMA_CH_RCR_PBL_LEN,
                                 pdata->rx_pbl);
    writereg(pdata->pAdapter, regval, FXGMAC_DMA_REG(channel, DMA_CH_RCR));
  }
#else
...
static void fxgmac_tx_mmc_int(struct fxgmac_pdata *pdata) {
  ...
  if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_TISR_TXOCTETCOUNT_GB_POS,
    ...
    stats->txoctetcount_gb += fxgmac_mmc_read(pdata, MMC_TXOCTETCOUNT_GB_LO);

  if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_TISR_TXFRAMECOUNT_GB_POS,
                          MMC_TISR_TXFRAMECOUNT_GB_LEN))
    stats->txframecount_gb += fxgmac_mmc_read(pdata, MMC_TXFRAMECOUNT_GB_LO);
  ...
  if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_TISR_TXUNDERFLOWERROR_POS,
    ...
    stats->txunderflowerror += fxgmac_mmc_read(pdata, MMC_TXUNDERFLOWERROR_LO);
  ...
  if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_TISR_TXEXCESSIVECOLLISIONFRAMES_POS,
...
static void fxgmac_rx_mmc_int(struct fxgmac_pdata *pdata) {
  ...
  if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RXFRAMECOUNT_GB_POS,
    ...
    stats->rxframecount_gb += fxgmac_mmc_read(pdata, MMC_RXFRAMECOUNT_GB_LO);

  if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RXOCTETCOUNT_GB_POS,
                          MMC_RISR_RXOCTETCOUNT_GB_LEN))
    stats->rxoctetcount_gb += fxgmac_mmc_read(pdata, MMC_RXOCTETCOUNT_GB_LO);
  ...
  if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RXOUTOFRANGETYPE_POS,
    ...
    stats->rxoutofrangetype += fxgmac_mmc_read(pdata, MMC_RXOUTOFRANGETYPE_LO);
  ...
  if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RXVLANFRAMES_GB_POS,
    ...
    stats->rxvlanframes_gb += fxgmac_mmc_read(pdata, MMC_RXVLANFRAMES_GB_LO);
...
static void fxgmac_read_mmc_stats(struct fxgmac_pdata *pdata) {
  ...
  stats->txsinglecollision_g += fxgmac_mmc_read(pdata, MMC_TXSINGLECOLLISION_G);
  ...
  stats->rxunicastframes_g += fxgmac_mmc_read(pdata, MMC_RXUNICASTFRAMES_G_LO);
  ...
  stats->rxreceiveerrorframe += fxgmac_mmc_read(pdata, MMC_RXRECEIVEERRORFRAME);
...
static void fxgmac_config_mmc(struct fxgmac_pdata *pdata) {
  ...
  // disable interrupts for rx Tcp and ip good pkt counter which is not read by
  // MMC counter process. regval = readreg(pdata->mac_regs + 0x800);
...
static u32 fxgmac_read_rss_options(struct fxgmac_pdata *pdata) {
  ...
  regval =
      FXGMAC_GET_REG_BITS(regval, MGMT_RSS_CTRL_OPT_POS, MGMT_RSS_CTRL_OPT_LEN);
...
static int fxgmac_read_rss_hash_key(struct fxgmac_pdata *pdata, u8 *key_buf) {
  ...
  while (key_regs--) {
    (*key) = cpu_to_be32(readreg(
        pdata->pAdapter,
        pdata->base_mem + (MGMT_RSS_KEY0 + key_regs * MGMT_RSS_KEY_REG_INC)));

    DBGPRINT(MP_LOUD,
             ("fxgmac_read_rss_hash_key: idx=%d, reg=%x, key=0x%08x\n",
              key_regs, MGMT_RSS_KEY0 + key_regs * MGMT_RSS_KEY_REG_INC,
              (u32)(*key)));
    key++;
  }
...
static int fxgmac_write_rss_lookup_table(struct fxgmac_pdata *pdata) {
  ...
  for (i = 0, j = 0; i < ARRAY_SIZE(pdata->rss_table); i++, j++) {
    if (j < MGMT_RSS_IDT_ENTRY_PER_REG) {
      ...
      ret = fxgmac_write_rss_reg(pdata, FXGMAC_RSS_LOOKUP_TABLE_TYPE,
                                 MGMT_RSS_IDT +
                                     (i / MGMT_RSS_IDT_ENTRY_PER_REG - 1) *
  ...
  if (j == MGMT_RSS_IDT_ENTRY_PER_REG) {
    // last IDT
    fxgmac_write_rss_reg(pdata, FXGMAC_RSS_LOOKUP_TABLE_TYPE,
...
static int fxgmac_set_rss_lookup_table(struct fxgmac_pdata *pdata,
  ...
  for (i = 0; i < ARRAY_SIZE(pdata->rss_table); i++) {
    ...
    pdata->rss_table[i] = FXGMAC_SET_REG_BITS(
        pdata->rss_table[i], MAC_RSSDR_DMCH_POS, MAC_RSSDR_DMCH_LEN, tval);
  }
...
static int fxgmac_enable_rss(struct fxgmac_pdata *pdata) {
  ...
  // 2022-04-19 xiaojiang comment
  // In Linux driver, it does not set the IDT table size, but windos implement
  // it, so linux driver follow it.
  ...
  /* Enable RSS */
  regval =
      FXGMAC_SET_REG_BITS(regval, MAC_RSSCR_RSSE_POS, MAC_RSSCR_RSSE_LEN, 1);
...
static int fxgmac_disable_rss(struct fxgmac_pdata *pdata) {
  ...
  regval = readreg(pdata->pAdapter, pdata->base_mem + MGMT_RSS_CTRL);
  regval =
      FXGMAC_SET_REG_BITS(regval, MAC_RSSCR_RSSE_POS, MAC_RSSCR_RSSE_LEN, 0);
...
//
static void fxgmac_update_aoe_ipv4addr(struct fxgmac_pdata *pdata,
                                       u8 *ip_addr) {
  ...
  if (ip_addr) {
    ...
    ipval = (ip_addr[0] << 24) | (ip_addr[1] << 16) | (ip_addr[2] << 8) |
            (ip_addr[3] << 0);
    DPRINTK("%s, covert IP dotted-addr %s to binary 0x%08x ok.\n", __FUNCTION__,
            ip_addr, cpu_to_be32(ipval));
  } else {
...
static int fxgmac_enable_arp_offload(struct fxgmac_pdata *pdata) {
  ...
  /* Enable arpoffload */
  regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_CR);
  regval = FXGMAC_SET_REG_BITS(regval, MAC_CR_ARPEN_POS, MAC_CR_ARPEN_LEN, 1);
  writereg(pdata->pAdapter, regval, pdata->mac_regs + MAC_CR);
...
static int fxgmac_disable_arp_offload(struct fxgmac_pdata *pdata) {
  ...
  /* disable arpoffload */
  regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_CR);
  regval = FXGMAC_SET_REG_BITS(regval, MAC_CR_ARPEN_POS, MAC_CR_ARPEN_LEN, 0);
  writereg(pdata->pAdapter, regval, pdata->mac_regs + MAC_CR);
 ...
 * parameters:
 *  index - 0~1, index to NS look up table. one entry of the lut is like this
 * |remote|solicited|target0|target1| remote_addr - ipv6 addr where fuxi gets
 * the NS solicitation pkt(request). in common, it is 0 to match any remote
 * machine. solicited_addr - the solicited node multicast group address which
 * fuxi computes and joins. target_addr1 - it is the target address in NS
 * solicitation pkt. target_addr2 - second target address, any address (with
 * last 6B same with target address?).
 */
static int
...
// static int fxgmac_set_ns_offload(struct nic_pdata* pdata,unsigned char index,
// PIPV6NSPARAMETERS PIPv6NSPara)

{
  ...
  for (i = 0; i < 16 / 4; i++) {
    Address[i] = (remote_addr[i * 4 + 0] << 24) |
                 ...
                 (remote_addr[i * 4 + 2] << 8) | (remote_addr[i * 4 + 3] << 0);
    writereg(pdata->pAdapter, Address[i],
             pdata->base_mem + 0X38 * index + NS_LUT_ROMOTE0 + 4 * i);
    if (Address[i]) {
      remote_not_zero = 1;
    }
    Address[i] =
        (target_addr1[i * 4 + 0] << 24) | (target_addr1[i * 4 + 1] << 16) |
        (target_addr1[i * 4 + 2] << 8) | (target_addr1[i * 4 + 3] << 0);
    writereg(pdata->pAdapter, Address[i],
             pdata->base_mem + 0X38 * index + NS_LUT_TARGET0 + 4 * i);
    Address[i] =
        (solicited_addr[i * 4 + 0] << 24) | (solicited_addr[i * 4 + 1] << 16) |
        (solicited_addr[i * 4 + 2] << 8) | (solicited_addr[i * 4 + 3] << 0);
    writereg(pdata->pAdapter, Address[i],
             pdata->base_mem + 0X38 * index + NS_LUT_SOLICITED0 + 4 * i);
    Address[i] =
        (target_addr2[i * 4 + 0] << 24) | (target_addr2[i * 4 + 1] << 16) |
        (target_addr2[i * 4 + 2] << 8) | (target_addr2[i * 4 + 3] << 0);
    writereg(pdata->pAdapter, Address[i],
             pdata->base_mem + 0X10 * index + NS_LUT_TARGET4 + 4 * i);
  }
  mac_addr_hi = (mac_addr[0] << 24) | (mac_addr[1] << 16) | (mac_addr[2] << 8) |
                (mac_addr[3] << 0);
  ...
  writereg(0X00033382,
           pdata->mac_regs - 0x2000 + 0X38 * 2 + NS_LUT_MAC_ADDR_CTL);
  ...
  writereg(0X00033381,
           pdata->mac_regs - 0x2000 + 0X38 * 3 + NS_LUT_MAC_ADDR_CTL);
...
static void fxgmac_update_ns_offload_ipv6addr(struct fxgmac_pdata *pdata,
  ...
  if (NULL ==
      fxgmac_get_netdev_ip6addr(
          pdata, target_addr1, solicited_addr,
          (FXGMAC_NS_IFA_LOCAL_LINK | FXGMAC_NS_IFA_GLOBAL_UNICAST) & param)) {
  ...
  DPRINTK("%s, Get net device MAC addr ok, ns_tab idx=%d, "
  ...
  fxgmac_set_ns_offload(pdata, pdata->expansion.ns_offload_tab_idx++,
                        remote_addr, solicited_addr, target_addr1, target_addr1,
                        mac_addr);
...
static int fxgmac_set_wake_pattern_mask(struct fxgmac_pdata *pdata,
  ...
  if (filter_index > 15 || register_index > 2) {
    DbgPrintF(MP_TRACE,
              "%s - Remote mask  pointer  is  over range, filter_index:%d, "
              "register_index:0x%x\n",
              __FUNCTION__, filter_index, register_index);
    return -1;
  }
  writereg(pdata->pAdapter, Data,
           pdata->base_mem + address_offset[filter_index][register_index]);
  return 0;
}
...
static u16 wol_crc16(u8 *pucframe, u16 uslen) {
  ...
  for (i = 0; i < uslen; i++) {
    ...
    next_crc.bits.bit_1 =
        crc.bits.bit_14 ^ next_crc.bits.bit_0 ^ rrpe_data.bits.bit_1;
    next_crc.bits.bit_2 =
        crc.bits.bit_13 ^ next_crc.bits.bit_1 ^ rrpe_data.bits.bit_2;
    next_crc.bits.bit_3 =
        crc.bits.bit_12 ^ next_crc.bits.bit_2 ^ rrpe_data.bits.bit_3;
    next_crc.bits.bit_4 =
        crc.bits.bit_11 ^ next_crc.bits.bit_3 ^ rrpe_data.bits.bit_4;
    next_crc.bits.bit_5 =
        crc.bits.bit_10 ^ next_crc.bits.bit_4 ^ rrpe_data.bits.bit_5;
    next_crc.bits.bit_6 =
        crc.bits.bit_9 ^ next_crc.bits.bit_5 ^ rrpe_data.bits.bit_6;
    next_crc.bits.bit_7 =
        crc.bits.bit_8 ^ next_crc.bits.bit_6 ^ rrpe_data.bits.bit_7;
...
static int fxgmac_set_wake_pattern(struct fxgmac_pdata *pdata,
  ...
  for (i = 0; i < total_cnt; i++) {
    ...
    for (j = 0; j < pdata->pattern[i].mask_size; j++) {
      for (z = 0; z < ((j == (MAX_PATTERN_SIZE / 8 - 1)) ? 7 : 8); z++) {
        if (pdata->pattern[i].mask_info[j] & (0x01 << z)) {
          mask[map_index] =
              pdata->pattern[i]
                  .pattern_info[pdata->pattern[i].pattern_offset + mask_index];
          map_index++;
        }
        mask_index++;
      }
    ...
    for (kp = 0, km = 0; kp < sizeof(pdata->pattern[i].pattern_info);
         kp += 16, km += 2) {
      ptdata = &pdata->pattern[i].pattern_info[kp];
      ptmask = &pdata->pattern[i].mask_info[km];
      DBGPRINT(MP_LOUD,
               ("\n    %02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x  "
                "%02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x  Mask %02x-%02x",
                ptdata[0], ptdata[1], ptdata[2], ptdata[3], ptdata[4],
                ptdata[5], ptdata[6], ptdata[7], ptdata[8], ptdata[9],
                ptdata[10], ptdata[11], ptdata[12], ptdata[13], ptdata[14],
                ptdata[15], ptmask[0], ptmask[1]));
    }
    // fxgmac_dump_buffer(mask, map_index, 2);
    DbgPrintF(MP_LOUD,
              "WritePatternToNic62 the %d patterns crc = %x mask length = %d, "
              "mask_offset=%x.\n",
 ...
 * and no needed to call disable for that, fxgmac_get_wake_packet_indication
 * will clear to normal once done.
 */
static void fxgmac_enable_wake_packet_indication(struct fxgmac_pdata *pdata,
  ...
  val_wpi_crtl0 = FXGMAC_SET_REG_BITS(
      val_wpi_crtl0, MGMT_WPI_CTRL0_WPI_MODE_POS, MGMT_WPI_CTRL0_WPI_MODE_LEN,
      (en ? MGMT_WPI_CTRL0_WPI_MODE_WR : MGMT_WPI_CTRL0_WPI_MODE_NORMAL));
 ...
 *      buf_size    size of the packet container. since HW limit to 14bits, ie
 * 16KB all together. output: wake_reason from HW, we can indentify 1)magic
 * packet, or 2)pattern(remote wake packet) or WAKE_REASON_HW_ERR indicates err
...
static void fxgmac_get_wake_packet_indication(struct fxgmac_pdata *pdata,
  ...
  *packet_size = 0;
  ...
  if (FXGMAC_GET_REG_BITS(regval, MAC_PMT_STA_MGKPRCVD_POS,
                          MAC_PMT_STA_MGKPRCVD_LEN)) {
    *wake_reason = WAKE_REASON_MAGIC;
    b_need_pkt = 1;
    DbgPrintF(MP_TRACE, "%s - waken up by Magic.\n", __FUNCTION__);
  } else if (FXGMAC_GET_REG_BITS(regval, MAC_PMT_STA_RWKPRCVD_POS,
  ...
  *packet_size =
      FXGMAC_GET_REG_BITS(val_wpi_crtl0, MGMT_WPI_CTRL0_WPI_PKT_LEN_POS,
  ...
  while ((0 == (val_wpi_crtl0 & MGMT_WPI_CTRL0_WPI_OP_DONE))) {
    if (i < data_len_dw) {
      regval =
          (u32)readreg(pdata->pAdapter, pdata->base_mem + MGMT_WPI_CTRL1_DATA);
...
static int fxgmac_enable_wake_link_change(struct fxgmac_pdata *pdata) {
  ...
  regval = readreg(pdata->pAdapter, pdata->base_mem + WOL_CTL);
  regval =
      FXGMAC_SET_REG_BITS(regval, WOL_LINKCHG_EN_POS, WOL_LINKCHG_EN_LEN, 1);
  writereg(pdata->pAdapter, regval, pdata->base_mem + WOL_CTL);
  return 0;
}
static int fxgmac_disable_wake_link_change(struct fxgmac_pdata *pdata) {
  ...
  regval = readreg(pdata->pAdapter, pdata->base_mem + WOL_CTL);
  regval =
      FXGMAC_SET_REG_BITS(regval, WOL_LINKCHG_EN_POS, WOL_LINKCHG_EN_LEN, 0);
  writereg(pdata->pAdapter, regval, pdata->base_mem + WOL_CTL);
  return 0;
}
...
static void fxgmac_enable_dma_interrupts(struct fxgmac_pdata *pdata) {
  ...
  for (i = 0; i < pdata->channel_count; i++, channel++) {
    ...
    */
    dma_ch_ier = FXGMAC_SET_REG_BITS(dma_ch_ier, DMA_CH_IER_FBEE_POS,
    ...
    if (channel->rx_ring) {
       ...
       *          per channel interrupts)
       */
      dma_ch_ier = FXGMAC_SET_REG_BITS(dma_ch_ier, DMA_CH_IER_RBUE_POS,
                                       DMA_CH_IER_RBUE_LEN, 1);
      // 2022-04-20 xiaojiang comment
      // windows driver set the per_channel_irq to be zero, Linux driver also
      // comment this, so comment it directly if (!pdata->per_channel_irq)
...
static void fxgmac_enable_mac_interrupts(struct fxgmac_pdata *pdata) {
  ...
  mac_ier = FXGMAC_SET_REG_BITS(mac_ier, MAC_IER_TSIE_POS, MAC_IER_TSIE_LEN, 1);
  ...
  if (pdata->base_mem) {
    ...
    *link_up = false;
    if (link_reg & MGMT_EPHY_CTRL_STA_EPHY_RELEASE) {
      ...
      if (speed)
        *speed = (link_reg & MGMT_EPHY_CTRL_STA_SPEED_MASK) >>
...
static int fxgmac_read_ephy_reg(struct fxgmac_pdata *pdata, u32 reg_id,
  ...
  if (0 == (regval & MAC_MDIO_ADDRESS_BUSY)) {
    ...
    if (data) {
      *data = regret;
      // DPRINTK("fxgmac_read_ephy_reg ok, reg=0x%02x, ctrl=0x%08x,
      // data=0x%08x\n", reg_id, regval, *data);
      return 0;
    } else {
      return -ENOBUFS;
...
static int fxgmac_write_ephy_mmd_reg(struct fxgmac_pdata *pdata, u32 reg_id,
  ...
  DPRINTK("  ctrl=0x%08x,", regval);
  DPRINTK("  data=0x%08x\n", data);
  // DbgPrintF(MP_TRACE, "fxgmac_write_ephy_mmd_reg id %d %s, ctrl=0x%08x,
  // data=0x%08x busy %d", reg_id, (regval & 0x1) ? "err" : "ok", regval, data,
  // busy);
...
static int fxgmac_read_ephy_mmd_reg(struct fxgmac_pdata* pdata, u32 reg_id, u32
mmd, u32* data)
{
    u32 regval = 0, regret;
    ...
    DPRINTK("fxgmac_read_ephy_mmd_reg id=0x%02x mmd %d err, busy=%d,
ctrl=0x%08x\n", reg_id, mmd, busy, regval);
    //DbgPrintF(MP_TRACE, "fxgmac_read_ephy_mmd_reg id=0x%02x err, busy=%d,
ctrl=0x%08x\n", reg_id, busy, regval); return -1;
}
#endif
*/
...
static void fxgmac_config_flow_control(struct fxgmac_pdata *pdata) {
  ...
  if (pdata->tx_pause || pdata->rx_pause) {
    regval = FXGMAC_SET_REG_BITS(regval, PHY_MII_ADVERTISE_PAUSE_POS,
                                 PHY_MII_ADVERTISE_PAUSE_LEN, 1);
    regval = FXGMAC_SET_REG_BITS(regval, PHY_MII_ADVERTISE_ASYPAUSE_POS,
...
static int fxgmac_set_ephy_autoneg_advertise(struct fxgmac_pdata *pdata,
  ...
  if (phy_ag_adv.auto_neg_en) {
    fxgmac_read_ephy_reg(pdata, REG_MII_BMCR, &regval);
    regval =
        FXGMAC_SET_REG_BITS(regval, PHY_CR_AUTOENG_POS, PHY_CR_AUTOENG_LEN, 1);
  } else {
    fxgmac_read_ephy_reg(pdata, REG_MII_BMCR, &regval);
    regval =
        FXGMAC_SET_REG_BITS(regval, PHY_CR_AUTOENG_POS, PHY_CR_AUTOENG_LEN, 0);
  }
  ...
  if (phy_ag_adv.full_1000m) {
    regval = FXGMAC_SET_REG_BITS(regval, PHY_MII_CTRL1000_1000FULL_POS,
  ...
  if (phy_ag_adv.half_1000m) {
    regval = FXGMAC_SET_REG_BITS(regval, PHY_MII_CTRL1000_1000HALF_POS,
  ...
  if (phy_ag_adv.full_100m) {
    regval = FXGMAC_SET_REG_BITS(regval, PHY_MII_ADVERTISE_100FULL_POS,
  ...
  if (phy_ag_adv.half_100m) {
    regval = FXGMAC_SET_REG_BITS(regval, PHY_MII_ADVERTISE_100HALF_POS,
  ...
  if (phy_ag_adv.full_10m) {
    regval = FXGMAC_SET_REG_BITS(regval, PHY_MII_ADVERTISE_10FULL_POS,
  ...
  if (phy_ag_adv.half_10m) {
    regval = FXGMAC_SET_REG_BITS(regval, PHY_MII_ADVERTISE_10HALF_POS,
...
static int fxgmac_phy_config(struct fxgmac_pdata *pdata) {
  ...
  case SPEED_1000:
    ...
    phy_ag_adv.full_100m = 1, phy_ag_adv.half_100m = 1, phy_ag_adv.full_10m = 1,
...
static void fxgmac_phy_eee_feature(struct fxgmac_pdata *pdata) {
  ...
  regval =
      FXGMAC_SET_REG_BITS(regval, DMA_SBMR_EN_LPI_POS, DMA_SBMR_EN_LPI_LEN, 1);
  ...
  regval =
      FXGMAC_SET_REG_BITS(regval, MAC_MS_TIC_POS, MAC_MS_TIC_LEN, MAC_MS_TIC);
...
void fxgmac_release_phy(struct fxgmac_pdata *pdata) {
  ...
  value = FXGMAC_SET_REG_BITS(value, PHY_MII_SPEC_CTRL_CRS_ON_POS,
                              PHY_MII_SPEC_CTRL_CRS_ON_LEN, 1); // set on crs on
  ...
  // VGA bandwidth, default is 2 after reset. Set to 0 to mitigate unstable
  // issue in 130m.
...
static int fxgmac_enable_int(struct fxgmac_channel *channel,
  ...
  dma_ch_ier =
      readreg(channel->pdata->pAdapter, FXGMAC_DMA_REG(channel, DMA_CH_IER));
  ...
  case FXGMAC_INT_DMA_CH_SR_TPS:
    dma_ch_ier = FXGMAC_SET_REG_BITS(dma_ch_ier, DMA_CH_IER_TXSE_POS,
                                     DMA_CH_IER_TXSE_LEN, 1);
    break;
  case FXGMAC_INT_DMA_CH_SR_TBU:
    dma_ch_ier = FXGMAC_SET_REG_BITS(dma_ch_ier, DMA_CH_IER_TBUE_POS,
...
static void fxgmac_disable_msix_one_interrupt(struct fxgmac_pdata *pdata,
  ...
  writereg(pdata->pAdapter, 0x01,
           pdata->base_mem + MSIX_TBL_BASE_ADDR + MSIX_TBL_MASK_OFFSET +
               intid * 16);
  // DbgPrintF(MP_LOUD, "%s - Ephy, MsgId %d is disabled.", __FUNCTION__,
  // IntId);
}
...
static int fxgmac_flush_tx_queues(struct fxgmac_pdata *pdata) {
  ...
  for (i = 0; i < pdata->tx_q_count; i++) {
    ...
    writereg(pdata->pAdapter, regval, FXGMAC_MTL_REG(pdata, i, MTL_Q_TQOMR));
    DPRINTK("fxgmac_flush_tx_queues, reg=0x%p,",
  ...
  for (i = 0; i < pdata->tx_q_count; i++) {
    ...
    do {
      ...
      regval = readreg(pdata->pAdapter, FXGMAC_MTL_REG(pdata, i, MTL_Q_TQOMR));
      regval =
          FXGMAC_GET_REG_BITS(regval, MTL_Q_TQOMR_FTQ_POS, MTL_Q_TQOMR_FTQ_LEN);

    } while (--count && regval);
    DPRINTK("fxgmac_flush_tx_queues wait... reg=0x%p,",
            FXGMAC_MTL_REG(pdata, i, MTL_Q_TQOMR));
    DPRINTK(" ... val=0x%08x\n", regval);
    if (regval) { /*(!count)*/
      return -EBUSY;
...
static void fxgmac_config_dma_bus(struct fxgmac_pdata *pdata) {
  ...
  regval = FXGMAC_SET_REG_BITS(regval, DMA_SBMR_EAME_POS, DMA_SBMR_EAME_LEN, 1);
  ...
  regval = FXGMAC_SET_REG_BITS(regval, DMA_SBMR_FB_POS, DMA_SBMR_FB_LEN, 0);
  regval =
      FXGMAC_SET_REG_BITS(regval, DMA_SBMR_BLEN_4_POS, DMA_SBMR_BLEN_4_LEN, 1);
  regval =
      FXGMAC_SET_REG_BITS(regval, DMA_SBMR_BLEN_8_POS, DMA_SBMR_BLEN_8_LEN, 1);
...
static void
fxgmac_link_speed_down_fix_shutdown_issue(struct fxgmac_pdata *pdata) {
  ...
  if ((ULONG)pdata->phy_speed != ((PMP_ADAPTER)pdata->pAdapter)->usLinkSpeed) {
    ...
    if (((PMP_ADAPTER)pdata->pAdapter)->RegParameter.LinkChgWol) {
      ...
      tick_interval =
          tick_count.QuadPart -
          ((PMP_ADAPTER)pdata->pAdapter)->D0_entry_tick_count.QuadPart;
      tick_interval *= tick_inc;
      tick_interval /= 10;

      /*DbgPrintF(MP_TRACE, "base tick %lld",
      ((PMP_ADAPTER)pdata->pAdapter)->D0_entry_tick_count.QuadPart);
      ...
      if (((regval & MGMT_EPHY_CTRL_STA_EPHY_RELEASE) &&
           (regval & MGMT_EPHY_CTRL_STA_EPHY_LINKUP)) ||
          ((regval & MGMT_EPHY_CTRL_STA_EPHY_RELEASE) &&
           !(regval & MGMT_EPHY_CTRL_STA_EPHY_LINKUP) &&
           (tick_interval < RESUME_MAX_TIME))) {
...
static void fxgmac_pre_powerdown(struct fxgmac_pdata *pdata, bool phyloopback) {
  ...
  // for phy cable loopback,it can't configure phy speed, it will cause os
  // resume again by link change although it has finished speed setting,
  if (!phyloopback) {
    fxgmac_read_ephy_reg(pdata, REG_MII_LPA, &regval);
    if (!FXGMAC_GET_REG_BITS(regval, PHY_MII_LINK_PARNTNER_10FULL_POS,
    ...
    suspend->resume->suspend process. The process of suspending again after
    resume is fast, and PHY auto-negotiation is not yet complete, so the
    auto-negotiation of PHY must be carried out again.Windows platforms and UEFI
    platforms do not need to auto-negotiate again, as they will not have such a
    process. When the Linux platform enters the s4 state, force speed to 10M.
    */
    regval = fxgmac_get_ephy_state(pdata);
  ...
  /* After enable OOB_WOL from efuse, mac will loopcheck phy status, and lead to
  panic sometimes. So we should disable it from powerup, enable it from power
  down.*/
...
#endif
{
  ...
  regval =
      FXGMAC_SET_REG_BITS(regval, LPW_CTRL_ASPM_LPW_EN_POS,
                          LPW_CTRL_ASPM_LPW_EN_LEN, 1); // Enable PCIE PM_L23.
#if 0
...
static int fxgmac_diag_sanity_check(struct fxgmac_pdata *pdata) {
  ...
  reg_q_val =
      readreg(pdata->pAdapter,
              FXGMAC_MTL_REG(pdata, 0 /* tx channe 0 */, 0x8 /*  0x2d08 */));
  if (!(reg_q_val & 0x10)) { // tx q is empty
    return 0;
  }
  reg_tail_val = readreg(pdata->pAdapter,
  ...
  if (cnt > 10) {
    ...
    if (reg_q_val & 0x10) { // double check
      DPRINTK("fxgmac, WARNing, tx Q status is 0x%x and tail keeps unchanged "
              "for %d times, 0x%x\n",
...
// context - pointer to struct nic_pdata.
{
  ...
  val_mgmt_intcrtl0 = FXGMAC_SET_REG_BITS(
      ...
      MGMT_INT_CTRL0_INT_MASK_LEN, MGMT_INT_CTRL0_INT_MASK_EX_PMT);
  ...
  writereg(pdata->pAdapter, 0x0,
           pdata->base_mem + MSIX_TBL_BASE_ADDR + MSIX_TBL_MASK_OFFSET +
               MSI_ID_PHY_OTHER * 16);
  readreg(pdata->pAdapter,
          pdata->base_mem + MGMT_WOL_CTRL); //  read  clear  wake up  reason
  // since Msix interrupt masked now, enable EPHY interrupt for case of link
  // change wakeup
  ...
  regval = FXGMAC_SET_REG_BITS(regval, PHY_INT_MASK_LINK_DOWN_POS,
                               PHY_INT_MASK_LINK_DOWN_LEN, 1);
  fxgmac_write_ephy_reg(pdata, REG_MII_INT_MASK, regval); // enable phy
                                                          // interrupt
#endif
...
static int fxgmac_suspend_txrx(struct fxgmac_pdata *pdata) {
  ...
  if (0 != (regval & MAC_DBG_STA_TX_BUSY)) {
    regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_DBG_STA);
    DbgPrintF(MP_WARN,
              "warning !!!timed out waiting for Tx MAC to stop regval %x\n",
              regval);
    return -1;
  }
  /* wait  empty Tx queue */
...
static void fxgmac_resume_int(struct fxgmac_pdata *pdata) {
  ...
  fxgmac_read_ephy_reg(pdata, REG_MII_INT_STATUS,
                       NULL); //  clear  phy interrupt
#else
  // hw_ops->read_ephy_reg(pdata, REG_MII_INT_STATUS, NULL);//     clear  phy
  // interrupt
  regval = FXGMAC_SET_REG_BITS(0, PHY_INT_MASK_LINK_UP_POS,
                               PHY_INT_MASK_LINK_UP_LEN, 1);
  regval = FXGMAC_SET_REG_BITS(regval, PHY_INT_MASK_LINK_DOWN_POS,
...
static void fxgmac_save_nonstick_reg(struct fxgmac_pdata *pdata) {
  ...
  dm_pci_read_config16(pdata->pdev, PCI_SUBSYSTEM_VENDOR_ID,
                       &pdata->expansion.SubVendorID);
  dm_pci_read_config16(pdata->pdev, PCI_SUBSYSTEM_ID,
                       &pdata->expansion.SubSystemID);

  dm_pci_read_config8(pdata->pdev, PCI_REVISION_ID,
                      &pdata->expansion.pci_revid);
  // dm_pci_read_config16(pdata->pdev, PCI_COMMAND, &pdata->pci_cmd_word);

  DbgPrintF(
      MP_TRACE,
      "VenId is %x, Devid is %x, SubId is %x, SubSysId is %x, Revid is %x.\n",
      pdata->expansion.pci_venid, pdata->expansion.pci_devid,
      pdata->expansion.SubVendorID, pdata->expansion.SubSystemID,
      pdata->expansion.pci_revid);
  ...
  DbgPrintF(MP_TRACE,
            ...
            "02x-%02x\nCFG%02x-%02x\nCFG%02x-%02x\nCFG%02x-%02x\nCFG%02x-%"
            "02x\nCFG%02x-%02x\n",
            __FUNCTION__, REG_PCI_COMMAND, pdata->expansion.cfg_pci_cmd,
...
static void fxgmac_restore_nonstick_reg(struct fxgmac_pdata *pdata) {
  u32 i;
  for (i = REG_PCIE_TRIGGER; i < MSI_PBA_REG; i += 4) {
    writereg(pdata->pAdapter, pdata->reg_nonstick[(i - REG_PCIE_TRIGGER) >> 2],
             pdata->base_mem + i);
  }
}
...
static int fxgmac_hw_exit(struct fxgmac_pdata *pdata) {
  ...
  if (PCI_LINK_CTRL_L1_STATUS == (pdata->pcie_link_status & 0x02)) {
    regval = FXGMAC_SET_REG_BITS(regval, PCI_LINK_CTRL_ASPM_CONTROL_POS,
  ...
  usleep_range_ex(pdata->pAdapter, 9000,
                  10000); // usleep_range_ex(pdata->pAdapter, 10, 15);
  ...
  regval = readreg(pdata->pAdapter, pdata->mac_regs + DMA_MR);
  DPRINTK("DMA_MR 0x%x\n", regval);
  regval = FXGMAC_SET_REG_BITS(regval, DMA_MR_SWR_POS, DMA_MR_SWR_LEN, 1);
  regval = FXGMAC_SET_REG_BITS(regval, DMA_MR_INTM_POS, DMA_MR_INTM_LEN, 0);
  writereg(pdata->pAdapter, regval, pdata->mac_regs + DMA_MR);
...
static int fxgmac_pcie_init(struct fxgmac_pdata *pdata, bool ltr_en,
                            ...
                            bool aspm_l0s_en) {
  // DbgPrintF(MP_TRACE, "%s ltr_en %d aspm_l1ss_en %d aspm_l1_en %d aspm_l0s_en
  // %d", __FUNCTION__, ltr_en, aspm_l1ss_en, aspm_l1_en, aspm_l0s_en);
  u32 regval = 0;
  u32 deviceid = 0;
  ...
  regval =
      FXGMAC_SET_REG_BITS(0, LTR_IDLE_ENTER_REQUIRE_POS,
                          LTR_IDLE_ENTER_REQUIRE_LEN, LTR_IDLE_ENTER_REQUIRE);
  regval = FXGMAC_SET_REG_BITS(regval, LTR_IDLE_ENTER_SCALE_POS,
                               LTR_IDLE_ENTER_SCALE_LEN, LTR_IDLE_ENTER_SCALE);
  regval = FXGMAC_SET_REG_BITS(regval, LTR_IDLE_ENTER_POS, LTR_IDLE_ENTER_LEN,
                               LTR_IDLE_ENTER_USVAL);
  regval = (regval << 16) + regval; /* snoopy + non-snoopy */
  writereg(pdata->pAdapter, regval, pdata->base_mem + LTR_IDLE_ENTER);

  regval = 0;
  regval =
      FXGMAC_SET_REG_BITS(0, LTR_IDLE_EXIT_REQUIRE_POS,
                          LTR_IDLE_EXIT_REQUIRE_LEN, LTR_IDLE_EXIT_REQUIRE);
  regval = FXGMAC_SET_REG_BITS(regval, LTR_IDLE_EXIT_SCALE_POS,
                               LTR_IDLE_EXIT_SCALE_LEN, LTR_IDLE_EXIT_SCALE);
  regval = FXGMAC_SET_REG_BITS(regval, LTR_IDLE_EXIT_POS, LTR_IDLE_EXIT_LEN,
                               LTR_IDLE_EXIT_USVAL);
  regval = (regval << 16) + regval; /* snoopy + non-snoopy */
  ...
  if (ltr_en) {
    ...
    regval = FXGMAC_SET_REG_BITS(regval, LTR_CTRL_IDLE_THRE_TIMER_POS,
  ...
  regval = FXGMAC_SET_REG_BITS(regval, LPW_CTRL_ASPM_L1_EN_POS,
                               LPW_CTRL_ASPM_L1_EN_LEN, aspm_l1_en ? 1 : 0);
  regval = FXGMAC_SET_REG_BITS(regval, LPW_CTRL_L1SS_EN_POS,
                               LPW_CTRL_L1SS_EN_LEN, aspm_l1ss_en ? 1 : 0);
  writereg(pdata->pAdapter, regval, pdata->base_mem + LPW_CTRL);
  ...
  if (FXGMAC_REV_01 == regval && PCI_DEVICE_ID_FUXI == deviceid) {
    ...
    regval = FXGMAC_SET_REG_BITS(regval, MGMT_SIGDET_DEGLITCH_DISABLE_POS,
                                 MGMT_SIGDET_DEGLITCH_DISABLE_LEN, 1);
...
static void fxgmac_clear_misc_int_status(struct fxgmac_pdata *pdata) {
  ...
  for (i = 0; i < q_count; i++) {
    ...
    regval = pdata->hw_ops.get_gmac_register(
        pdata, pdata->mac_regs + MTL_Q_INT_CTL_SR + i * MTL_Q_INC);
    pdata->hw_ops.set_gmac_register(
        pdata, pdata->mac_regs + MTL_Q_INT_CTL_SR + i * MTL_Q_INC, regval);
  }

  /* MTL_ECC_Interrupt_Status, write 1 clear */
  regval =
      pdata->hw_ops.get_gmac_register(pdata, pdata->mac_regs + MTL_ECC_INT_SR);
  pdata->hw_ops.set_gmac_register(pdata, pdata->mac_regs + MTL_ECC_INT_SR,
                                  regval);

  /* DMA_ECC_Interrupt_Status, write 1 clear */
  regval =
      pdata->hw_ops.get_gmac_register(pdata, pdata->mac_regs + DMA_ECC_INT_SR);
