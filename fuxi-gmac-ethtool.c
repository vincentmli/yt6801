...
static void fxgmac_ethtool_get_drvinfo(struct net_device *netdev,
  ...
  userver = FXGMAC_GET_REG_BITS(ver, MAC_VR_USERVER_POS, MAC_VR_USERVER_LEN);
  snprintf(drvinfo->fw_version, sizeof(drvinfo->fw_version), "S.D.U: %x.%x.%x",
           sver, devid, userver);
}
...
static void fxgmac_ethtool_get_channels(struct net_device *netdev,
  ...
  if (channel->combined_count == 1)
    return;
  DPRINTK("fxmac rss, get channels max=(combined %d,other %d),count(combined "
          "%d,other %d)\n",
          channel->max_combined, channel->max_other, channel->combined_count,
          channel->other_count);
#endif
...
static int fxgmac_ethtool_get_coalesce(
...
#endif
{
   ...
   * we need to modify the supported_coalesce_params of fxgmac_ethtool_ops
   * synchronously
   */
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0) ||                         \
     RHEL_RELEASE_CODE >= RHEL_RELEASE_VERSION(9, 1))
static int fxgmac_ethtool_set_coalesce(
    struct net_device *netdev, struct ethtool_coalesce *ec,
    struct kernel_ethtool_coalesce *kernel_coal, struct netlink_ext_ack *extack)
#else
...
static int fxgmac_get_rss_hash_opts(struct fxgmac_pdata *pdata,
  ...
  case UDP_V4_FLOW:
    if (((TCP_V4_FLOW == (cmd->flow_type)) &&
         (FXGMAC_GET_REG_BITS(pdata->rss_options, MAC_RSSCR_TCP4TE_POS,
                              MAC_RSSCR_TCP4TE_LEN))) ||
        ((UDP_V4_FLOW == (cmd->flow_type)) &&
         (FXGMAC_GET_REG_BITS(pdata->rss_options, MAC_RSSCR_UDP4TE_POS,
  ...
  case IPV4_FLOW:
    if (((TCP_V4_FLOW == (cmd->flow_type)) &&
        ...
        ((UDP_V4_FLOW == (cmd->flow_type)) &&
         (FXGMAC_GET_REG_BITS(pdata->rss_options, MAC_RSSCR_UDP4TE_POS,
                              MAC_RSSCR_UDP4TE_LEN))) ||
        (FXGMAC_GET_REG_BITS(pdata->rss_options, MAC_RSSCR_IP4TE_POS,
                             MAC_RSSCR_IP4TE_LEN))) {
  ...
  case UDP_V6_FLOW:
    if (((TCP_V6_FLOW == (cmd->flow_type)) &&
         (FXGMAC_GET_REG_BITS(pdata->rss_options, MAC_RSSCR_TCP6TE_POS,
                              MAC_RSSCR_TCP6TE_LEN))) ||
        ((UDP_V6_FLOW == (cmd->flow_type)) &&
         (FXGMAC_GET_REG_BITS(pdata->rss_options, MAC_RSSCR_UDP6TE_POS,
  ...
  case IPV6_FLOW:
    if (((TCP_V6_FLOW == (cmd->flow_type)) &&
        ...
        ((UDP_V6_FLOW == (cmd->flow_type)) &&
         (FXGMAC_GET_REG_BITS(pdata->rss_options, MAC_RSSCR_UDP6TE_POS,
                              MAC_RSSCR_UDP6TE_LEN))) ||
        (FXGMAC_GET_REG_BITS(pdata->rss_options, MAC_RSSCR_IP6TE_POS,
                             MAC_RSSCR_IP6TE_LEN))) {
      cmd->data |= RXH_IP_SRC | RXH_IP_DST;
    }
    break;
...
static int fxgmac_set_rss_hash_opt(struct fxgmac_pdata *pdata,
  ...
  case TCP_V6_FLOW:
    ...
    if (TCP_V4_FLOW == (nfc->flow_type)) {
      ...
      rssopt = FXGMAC_SET_REG_BITS(rssopt, MAC_RSSCR_TCP4TE_POS,
    ...
    if (TCP_V6_FLOW == (nfc->flow_type)) {
      ...
      rssopt = FXGMAC_SET_REG_BITS(rssopt, MAC_RSSCR_TCP6TE_POS,
  ...
  case UDP_V4_FLOW:
    ...
    case (RXH_L4_B_0_1 | RXH_L4_B_2_3):
      rssopt = FXGMAC_SET_REG_BITS(rssopt, MAC_RSSCR_UDP4TE_POS,
  ...
  case UDP_V6_FLOW:
    ...
    case 0:
      break;
    case (RXH_L4_B_0_1 | RXH_L4_B_2_3):
      rssopt = FXGMAC_SET_REG_BITS(rssopt, MAC_RSSCR_UDP6TE_POS,
  ...
  if (rssopt != pdata->rss_options) {
    if ((rssopt & UDP_RSS_FLAGS) && !(pdata->rss_options & UDP_RSS_FLAGS)) {
...
static void fxgmac_set_pattern_data(struct fxgmac_pdata *pdata) {
  ...
  if (pdata->expansion.wol & WAKE_ARP) {
    ...
    packet.ar_op = 0x1 << 8; // 1 is arp request,2 is arp replay, 3 is rarp
                             // request, 4 is rarp replay
...
void fxgmac_config_wol(struct fxgmac_pdata *pdata, int en) {
  /* enable or disable WOL. this function only set wake-up type, and power
   * related configure will be in other place, see power management.
   */
  struct fxgmac_hw_ops *hw_ops = &pdata->hw_ops;
  if (!pdata->hw_feat.rwk) {
  ...
  if (en) {
    ...
    if (pdata->expansion.wol & WAKE_UCAST ||
        ...
        pdata->expansion.wol & WAKE_BCAST || pdata->expansion.wol & WAKE_ARP) {
...
static int fxgmac_set_link_ksettings(struct net_device *netdev,
  ...
  if (pdata->phy_autoeng ||
    ...
    if (ret < 0)
      return ret;
    adv = FXGMAC_SET_REG_BITS(adv, PHY_CR_RE_AUTOENG_POS, PHY_CR_RE_AUTOENG_LEN,
                              1);
    ret = hw_ops->write_ephy_reg(pdata, REG_MII_BMCR, adv);
    if (ret < 0)
      return ret;
...
static int fxgmac_set_pauseparam(struct net_device *netdev,
  ...
  if (pause->autoneg) {
    ...
    adv = FXGMAC_SET_REG_BITS(adv, PHY_MII_ADVERTISE_PAUSE_POS,
                              PHY_MII_ADVERTISE_PAUSE_LEN, enable_pause);
    adv = FXGMAC_SET_REG_BITS(adv, PHY_MII_ADVERTISE_ASYPAUSE_POS,
                              PHY_MII_ADVERTISE_ASYPAUSE_LEN, enable_pause);
    ret = hw_ops->write_ephy_reg(pdata, REG_MII_ADVERTISE, adv);
    if (ret < 0) {
      return ret;
    }
...
static int fxgmac_ethtool_reset(struct net_device *netdev, u32 *flag) {
  ...
  case ETH_RESET_PHY:
    ...
    val = FXGMAC_SET_REG_BITS(val, PHY_CR_POWER_POS, PHY_CR_POWER_LEN,
                              PHY_POWER_DOWN);
